...
export function WorkflowDetailTabs({
	...
	const eventByNodeId = useMemo(() => {
		const map = new Map<string, DaprExecutionEvent>();
	...
	const graphNodeLabelLookup = useMemo(() => {
					...
					<ResizablePanel defaultSize={selectedEvent ? 65 : 100} minSize={40}>
						<WorkflowDefinitionGraph
							...
							onNodeSelect={(nodeId) => {
								...
								const direct = eventByNodeId.get(nodeId);
								if (direct) {
									setSelectedEvent(direct);
									return;
								}
								const label = graphNodeLabelLookup.get(nodeId);
								const fallback =
									[...events]
										.reverse()
										.find(
											(event) =>
												event.name === label ||
												event.metadata?.nodeName === label ||
												event.metadata?.activityName === label,
										) ?? null;
