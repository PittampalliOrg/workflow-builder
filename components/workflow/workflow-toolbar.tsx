...
function getMissingRequiredFields(
	...
	return nodes
		.map((n) => getNodeMissingFields(n, findActionById))
...
function getMissingIntegrations(
	...
	const userPluginTypes = new Set(userIntegrations.map((i) => i.pieceName));
...
function useWorkflowHandlers({
	...
	const handleExecute = async () => {
		...
		const brokenRefs = getBrokenTemplateReferences(nodes, findActionById);
		const missingFields = getMissingRequiredFields(nodes, findActionById);
		const missingIntegrations = getMissingIntegrations(
			nodes,
			userIntegrations,
			findActionById,
			integrationLabels,
