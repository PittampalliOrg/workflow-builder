...
const getIntegrationFromActionType = (
	...
	getIntegration: ReturnType<typeof usePiecesCatalog>["getIntegration"],
): string => {
	...
	// Look up in pieces catalog
	const action = findActionById(actionType);
	if (action?.integration) {
		const piece = getIntegration(action.integration);
		return piece?.label || action.integration;
	}
...
const requiresIntegration = (
	...
	const systemActionsRequiringIntegration = ["system/database-query"];
...
const getProviderLogo = (
	...
	// Look up action in pieces catalog and use the piece logo if available
	const action = findActionById(actionType);
	if (action) {
		const piece = getIntegration(action.integration);
		if (piece?.logoUrl) {
