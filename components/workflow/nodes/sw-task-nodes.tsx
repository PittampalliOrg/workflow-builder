...
type SWNodeStatus = SWNodeData["status"];
...
const StatusBadge = ({ status }: { status?: SWNodeStatus }) => {
	...
	return (
		...
		>
			...
			{status === "skipped" && (
				<CircleOff className="size-3.5 text-white" strokeWidth={1.5} />
			)}
		</div>
	);
};
...
function makeTaskNode(
	...
	const TaskNode = memo(({ data, selected, id }: SWNodeProps) => {
		...
		const config =
			data.taskConfig ||
			(data as unknown as { config?: Record<string, unknown> }).config ||
			{};
		const subtitle =
			options?.subtitle?.(config) || data.description || displayName;
			...
			>
				...
				<div className="flex flex-col items-center justify-center gap-3 p-6">
					...
					<div className="flex flex-col items-center gap-1 text-center">
						...
						{subtitle && (
							<NodeDescription className="text-xs">{subtitle}</NodeDescription>
						)}
					</div>
				</div>
			</Node>
...
export const StartNode = makeTaskNode("Start", Play, "text-blue-500", {
	...
	subtitle: (config) =>
		(config.document as { title?: string })?.title || "Workflow start",
});
...
export const CallNode = makeTaskNode("Call", Globe, "text-amber-300", {
	subtitle: (config) => {
		...
		if (call === "http") {
			const args = config.with as
				| { method?: string; endpoint?: unknown }
				| undefined;
...
export const EmitNode = makeTaskNode("Emit", Send, "text-green-400", {
	subtitle: (config) => {
		const emit = config.emit as
			| { event?: { with?: { type?: string } } }
			| undefined;
...
export const DoNode = makeTaskNode("Do", Layers, "text-teal-400", {
	subtitle: (config) => {
		...
		if (tasks)
			return `${tasks.length} sub-task${tasks.length !== 1 ? "s" : ""}`;
