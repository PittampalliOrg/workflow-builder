/**
 * Upsert the "Durable Agent Long Flow Validation" workflow from its spec-first JSON file.
 *
 * Usage:
 *   DATABASE_URL=... node scripts/upsert-durable-agent-long-flow-validation-workflow.mjs
 */
...
const WORKFLOW_JSON_PATH = path.resolve(
	process.cwd(),
	'services/durable-agent/durable-agent-long-flow-validation.workflow.json'
);
...
main().catch((error) => {
	console.error('[upsert-durable-agent-long-flow-validation] Error:', error);
	process.exitCode = 1;
});
