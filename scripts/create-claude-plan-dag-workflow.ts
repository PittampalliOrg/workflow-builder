...
const DEFAULT_PLAN_PROMPT = `Add a "Copy Workflow ID" button to the workflow editor toolbar.

Feature spec:
- Add a small button (clipboard icon) next to the workflow name in the editor header/toolbar.
- Clicking it copies the current workflow ID to the clipboard using navigator.clipboard.writeText().
- Show a brief toast notification on success ("Workflow ID copied").
- Use the existing shadcn/ui toast and button components — do not add new dependencies.

Constraints:
1) Return tasks as a DAG with explicit blockedBy dependencies.
2) Keep tasks concrete and scoped to file-level work.
3) Include a final validation gate: type-check and lint/fix.
4) Do NOT include E2E or Playwright tests — the sandbox has no running app server.
5) Do NOT include unit tests unless a pure utility function is added.
6) Target ≤ 6 tasks total (implementation + validation).`;
...
function parseArgs(argv: string[]): Args {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (arg === "--user-email") {
			...
			i++;
		} else if (arg === "--sandbox-template") {
			sandboxTemplate = argv[i + 1] || sandboxTemplate;
			i++;
		}
	}
...
function buildWorkflowGraph(input: {
	...
	const featureBranchTemplate = `feature/ai-update-{{@${profileId}:Workspace Profile.executionId}}`;
	...
	const prConfig: Record<string, string> = {
		...
		title: `AI Automated Updates ({{@${profileId}:Workspace Profile.executionId}})`,
