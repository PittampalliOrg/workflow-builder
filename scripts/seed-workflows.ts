...
async function seedGeneratorCriticShowcases(params: {
	...
	await ensureCliShowcaseAgentFor(params.sqlClient, params.userId, params.projectId, {
		...
		const GAN_CRITIC_PERSONA =
			...
			"OUTPUT CONTRACT (a machine parses your FINAL message to decide whether the loop stops \u2014 this is critical): your FINAL message MUST be EXACTLY the strict JSON verdict object and NOTHING ELSE. It MUST start with `{` and end with `}`. Do NOT add any preamble, 'Summary of findings', explanation, or commentary before or after it, and do NOT wrap it in ``` markdown fences. Put ALL of your reasoning INSIDE the JSON's `feedback` field. If you emit any text outside the JSON, the harness cannot read your verdict and the loop will run forever.";
