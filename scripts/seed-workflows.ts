...
import {
	...
	ensureKimiAgent as ensureKimiK3AnimationAgent,
	WORKFLOW_DESCRIPTION as KIMI_K3_ANIMATION_WORKFLOW_DESCRIPTION,
...
const CLI_RUNTIME_DESCRIPTORS: readonly CliRuntimeDescriptor[] = [
	{
		runtime: "codex-cli",
		label: "Codex CLI",
	},
...
const CLI_RUNTIME_OPTIONS = CLI_RUNTIME_DESCRIPTORS.map(
	(item) => ({
		label: item.label,
		value: item.runtime,
	}),
...
async function ensureShowcaseAgent(
	...
	await sqlClient`
		...
		values (${versionId}, ${agentId}, ${1}, ${sqlClient.json(config as postgres.JSONValue)}, ${configHash})`;
...
async function ensureCliShowcaseAgentFor(
	...
	if (existing.length && existing[0].current_version_id) {
		...
		const cur = await sqlClient<
			{ config_hash: string | null; config_type: string | null }[]
		>`
		...
		if (
			cur.length &&
			cur[0].config_hash === configHash &&
			cur[0].config_type === "object"
		) {
			return slug;
		}
		...
		await sqlClient`
			insert into agent_versions (id, agent_id, version, config, config_hash)
			values (${newVersionId}, ${agentId}, ${nextVersion}, ${sqlClient.json(config as postgres.JSONValue)}, ${configHash})`;
...
async function seedWorkflow() {
	...
	try {
		...
		await upsertRawWorkflow({
			db,
			workflowId: KIMI_K3_ANIMATION_WORKFLOW_ID,
			name: KIMI_K3_ANIMATION_WORKFLOW_NAME,
			description: KIMI_K3_ANIMATION_WORKFLOW_DESCRIPTION,
			userId,
			projectId,
