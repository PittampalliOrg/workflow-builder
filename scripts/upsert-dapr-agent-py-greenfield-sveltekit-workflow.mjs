/**
 * Upsert the "Dapr Agent Py Greenfield SvelteKit" workflow from its spec-first JSON file.
 *
 * Usage:
 *   DATABASE_URL=... node scripts/upsert-dapr-agent-py-greenfield-sveltekit-workflow.mjs
 */
...
const WORKFLOW_JSON_PATH = path.resolve(
  process.cwd(),
  "services/dapr-agent-py/dapr-agent-py-greenfield-sveltekit.workflow.json",
);
...
async function resolveOwner(sql, existingWorkflow, userEmail) {
  ...
  const rows =
    await sql`select pm.user_id, pm.project_id from project_members pm order by pm.created_at asc limit 1`;
  if (rows[0]?.user_id)
    return { userId: rows[0].user_id, projectId: rows[0].project_id || null };
  const userRows =
    await sql`select id as user_id from users order by created_at asc limit 1`;
...
async function main() {
  ...
  try {
    ...
    const existing =
      await sql`select id, user_id, project_id from workflows where id = ${workflowId} limit 1`;
    ...
    if (existing[0]) {
      await sql`
				update workflows set
					...
					nodes = ${sql.json(wf.nodes)}, edges = ${sql.json(wf.edges)},
					visibility = ${wf.visibility || "public"},
					spec = ${sql.json(wf.spec)},
					updated_at = now()
				where id = ${workflowId}`;
      console.log(
        JSON.stringify({ workflowId, created: false, name: wf.name }, null, 2),
      );
    } else {
      await sql`
				insert into workflows (id, name, description, nodes, edges, visibility, spec, user_id, project_id, created_at, updated_at)
				values (${workflowId}, ${wf.name}, ${wf.description},
					${sql.json(wf.nodes)}, ${sql.json(wf.edges)},
					${wf.visibility || "public"}, ${sql.json(wf.spec)},
					${owner.userId}, ${owner.projectId}, now(), now())`;
      console.log(
        JSON.stringify({ workflowId, created: true, name: wf.name }, null, 2),
      );
    }
  } finally {
...
main().catch((error) => {
  console.error("[upsert-dapr-agent-py-greenfield-sveltekit] Error:", error);
  process.exitCode = 1;
});
