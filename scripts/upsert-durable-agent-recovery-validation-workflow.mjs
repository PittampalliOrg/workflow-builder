/**
 * Upsert the "Durable Agent Recovery Validation" workflow from its spec-first JSON file.
 *
 * Usage:
 *   DATABASE_URL=... node scripts/upsert-durable-agent-recovery-validation-workflow.mjs
 */
...
const WORKFLOW_JSON_PATH = path.resolve(
	process.cwd(),
	'services/durable-agent/durable-agent-recovery-validation.workflow.json'
);
...
main().catch((error) => {
	console.error('[upsert-durable-agent-recovery-validation] Error:', error);
	process.exitCode = 1;
});
