/**
 * Upsert the "Claude Code Sandbox Prompt Test" workflow from its spec-first JSON file.
 *
 * Usage:
 *   DATABASE_URL=... node scripts/upsert-claude-code-sandbox-prompt-test-workflow.mjs
 */
...
const WORKFLOW_JSON_PATH = path.resolve(
	process.cwd(),
	'services/claude-code-agent/claude-code-sandbox-prompt-test.workflow.json'
);
...
main().catch((error) => {
	console.error('[upsert-claude-code-sandbox-prompt-test] Error:', error);
	process.exitCode = 1;
});
