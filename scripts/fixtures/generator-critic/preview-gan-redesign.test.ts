...
const fixturePath = resolve(
	process.cwd(),
	'scripts/fixtures/generator-critic/preview-gan-redesign.json'
);
...
describe('preview-gan-redesign fixture', () => {
	...
	it('shares the contract via a /sandbox/work file and grades against it (no cross-node parseJson refs in the loop)', () => {
		...
		expect(plan.with.body.prompt).toContain('SYNC_TOKEN=');
		expect(plan.with.body.prompt).toContain('x-sync-token: $SYNC_TOKEN');
	});
