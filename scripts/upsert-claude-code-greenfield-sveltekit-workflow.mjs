/**
 * Upsert the "Claude Code Greenfield SvelteKit" workflow from its spec-first JSON file.
 *
 * Usage:
 *   DATABASE_URL=... node scripts/upsert-claude-code-greenfield-sveltekit-workflow.mjs
 */
...
const WORKFLOW_JSON_PATH = path.resolve(
	process.cwd(),
	'services/claude-code-agent/claude-code-greenfield-sveltekit.workflow.json'
);
...
async function resolveOwner(sql, existingWorkflow, userEmail) {
	if (existingWorkflow?.user_id) {
		return { userId: existingWorkflow.user_id, projectId: existingWorkflow.project_id || null };
	}
	if (userEmail) {
		...
		if (rows[0]?.user_id) return { userId: rows[0].user_id, projectId: rows[0].project_id || null };
	}
	const rows = await sql`select pm.user_id, pm.project_id from project_members pm order by pm.created_at asc limit 1`;
	if (rows[0]?.user_id) return { userId: rows[0].user_id, projectId: rows[0].project_id || null };
...
async function main() {
	...
	try {
		...
		const existing = await sql`select id, user_id, project_id from workflows where id = ${workflowId} limit 1`;
...
main().catch((error) => {
	console.error('[upsert-claude-code-greenfield-sveltekit] Error:', error);
	process.exitCode = 1;
});
