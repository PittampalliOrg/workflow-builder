...
async function syncOAuthApps() {
  ...
  try {
    // Get or create default platform (idempotent)
    let allPlatforms = await db.select().from(platforms).limit(1);
    if (allPlatforms.length === 0) {
      console.log("No platform found. Creating default platform...");
      const now = new Date();
      ...
      allPlatforms = await db.select().from(platforms).limit(1);
