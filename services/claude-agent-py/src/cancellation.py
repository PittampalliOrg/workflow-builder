"""Cooperative session-cancellation persistence for claude-agent-py.

Mirrors dapr-agent-py's cancellation surface so the BFF lifecycle controller +
benchmark cascade can cooperatively stop a claude run: the raise-event endpoint
persists a ``session-cancel:{instance}`` flag when a terminal control event
(``session.terminate`` / ``user.interrupt``) arrives, and the session workflow
reads that flag between turns via ``check_cancellation_activity``.

Lives in its own module so both main.py (write side + activity registration) and
session_workflow.py (the between-turn check) can import it without a cycle.
"""
