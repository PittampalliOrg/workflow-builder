 ...
 */
export async function discoverMcpTools(configs?: McpServerConfig[]): Promise<{
	...
	try {
		...
		const MCPClient =
			(mastraMcp as any).MCPClient ?? (mastraMcp as any).default?.MCPClient;

		if (!MCPClient) {
			console.warn(
				"[mcp-client] @mastra/mcp loaded but MCPClient class not found",
			);
			return { tools: {}, disconnect: async () => {} };
		}
