...
} from "./workflow/agent-workflow.js";
import {
	createOrchestrationWorkflow,
	type OrchestrationActivities,
} from "./workflow/orchestration-workflow.js";
...
export class DurableAgent {
	...
	readonly orchestrationWorkflow: ReturnType<
		typeof createOrchestrationWorkflow
	> | null;
	...
	constructor(options: DurableAgentOptions) {
		...
		const stateKey = options.state?.stateKey ?? `${this.name}:workflow_state`;
		...
		if (options.execution?.orchestrationMode) {
			switch (options.execution.orchestrationMode) {
				case OrchestrationMode.ROUNDROBIN:
					this.orchestrationStrategy = new RoundRobinOrchestrationStrategy();
					break;
	...
	private _createGetAvailableAgents() {
		...
		return async function getAvailableAgents(
			...
			for (const [name, meta] of Object.entries(agentsMetadata)) {
				const role = (meta as AgentRegistryEntry).agent?.role ?? "Unknown role";
	...
	private _createProcessOrchestrationResponse(strategy: OrchestrationStrategy) {
	...
	private _createShouldContinueOrchestration(strategy: OrchestrationStrategy) {
	...
	private _createInvokeRemoteAgent() {
		...
		return async function invokeRemoteAgent(
			...
			try {
				const result = await daprClient.invoker.invoke(payload.appId, "run", {
					method: "POST",
