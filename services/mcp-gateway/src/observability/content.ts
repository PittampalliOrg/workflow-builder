/**
 * Request/response content capture for MCP gateway spans.
 *
 * Emits OpenInference `input.value` / `output.value` attributes on the active
 * span so the Service Graph drill-down drawer can show MCP tool request and
 * response content. Values are deep-redacted before serialization.
 */
