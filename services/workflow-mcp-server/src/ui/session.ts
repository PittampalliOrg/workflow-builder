  ...
  private async buildModel(): Promise<UiModel> {
    ...
    return {
      ...
      nodeTypes: [
        ...
        { value: "openshell-durable-agent", label: "OpenShell Durable Agent" },
  ...
  async handleEvent(
    ...
    try {
      switch (event.type) {
        ...
        case "node.add": {
          const isOpenShellDurableAgent =
            event.nodeType === "openshell-durable-agent";
          const nodeType = isOpenShellDurableAgent ? "action" : event.nodeType;
          const label =
            event.label ??
            (isOpenShellDurableAgent
