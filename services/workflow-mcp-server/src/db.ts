...
const REMOVED_AGENT_ACTION_PREFIXES = new Set([
  "agent",
  "durable",
  "mastra",
  "ms-agent",
...
function actionMatchesSearch(action: ActionSummary, search?: string): boolean {
...
export async function listAvailableActions(
  ...
  for (const row of funcResult.rows) {
    if (isRemovedAgentAction(row.slug)) continue;
    results.push({
      slug: row.slug,
      name: row.name,
  ...
  for (const row of pieceResult.rows) {
    ...
    for (const [key, action] of Object.entries(actions)) {
      ...
      if (isRemovedAgentAction(slug)) continue;
      if (
        search &&
