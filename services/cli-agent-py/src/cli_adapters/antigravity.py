...
class AntigravityAdapter(CliAdapter):
    ...
    def map_transcript_entry(
        ...
        if text:
            ...
            events.append(event)
        # No estimation fallback: usage events are emitted only from
        # provider-reported usage (_agy_usage). The former 4-chars/token
        # transcript estimate existed because AGY never surfaced token counts;
        # antigravity is no longer a used runtime.
        if usage:
