/**
 * Static 48-piece bundle (the converged piece-runtime image).
 *
 * These top-level imports make esbuild emit an eager `require("@activepieces/piece-*")`
 * for EVERY piece at module load — that's the single-container bundle, and the reason a
 * per-piece image must NOT load this file. In a per-piece / base image build, build.mjs
 * (BUILD_VARIANT=single) resolves this module to `piece-registry.empty.ts` instead, so
 * none of these requires land in dist. See docs/per-piece-runtime-images.md.
 */
