...
const replyToMcpClient = createAction({
	...
	async run(context) {
		if (!INTERNAL_API_TOKEN) {
			throw new Error(
				"INTERNAL_API_TOKEN is not configured on the piece-runtime",
			);
		}
		...
		const rawResponse = context.propsValue.response;
		let responseValue: unknown;
		if (typeof rawResponse === "object" && rawResponse !== null) {
			// Already an object (e.g. resolved from template variable)
