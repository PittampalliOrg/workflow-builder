/**
 * Bundle-free piece catalog row-builder.
 *
 * Extracted from metadata-catalog.ts so a single piece's catalog row can be
 * built WITHOUT importing piece-registry.ts (which eagerly loads all 47 bundled
 * `@activepieces/piece-*` packages) or extensions/index.ts (which eagerly imports
 * the microsoft-onedrive piece). The catalog snapshot generator
 * (gen-catalog-snapshot.ts) imports `buildPieceCatalogRow` against ONE
 * isolated-installed piece — loading the whole bundle would crash there.
 *
 * `extensionsFor` is INJECTED (default: none). The bundled runtime
 * (metadata-catalog.ts) injects the real registry so digests for bundled pieces
 * stay byte-identical; the generator omits it (available-only pieces never carry
 * bespoke extensions).
 */
...
// biome-ignore lint/suspicious/noExplicitAny: Action is generic over auth + props; we erase for the lookup.
export type ExtensionLookup = (pieceName: string) => Action<any, any>[];
...
const NO_EXTENSIONS: ExtensionLookup = () => [];
...
export function buildPieceCatalogRow(input: {
	...
	/**
	 * Explicit version override. The piece's own declared `version` still wins;
	 * this is consulted before the on-disk `packageVersion` scan. The snapshot
	 * generator passes the isolated-installed package.json version (the scan
	 * can't see a temp-dir install), keeping version + digest consistent.
	 */
	version?: string | null;
	/**
	 * Supplementary actions to register alongside the piece's own (microsoft-
	 * onedrive extensions in the bundled runtime). Default: none. The bundled
	 * metadata-catalog.ts injects the real registry to keep digests identical;
	 * the snapshot generator omits it.
	 */
	extensionsFor?: ExtensionLookup;
}): PieceCatalogRow {
	const normalizedName = normalizePieceName(input.pieceName);
	const extensionsFor = input.extensionsFor ?? NO_EXTENSIONS;
	...
	const base = {
		...
		version:
			stringOrNull(rawPiece.version) ??
			stringOrNull(input.version) ??
			packageVersion(normalizedName) ??
			"0.0.0",
