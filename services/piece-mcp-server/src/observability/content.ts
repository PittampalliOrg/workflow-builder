/**
 * Request/response content capture for piece MCP server spans.
 *
 * Emits OpenInference `input.value` / `output.value` attributes on the active
 * span so the Service Graph drill-down drawer can show MCP requests, action
 * arguments, and action results. Values are deep-redacted before serialization.
 */
