...
class OpenShellDurableAgent(DurableAgent):
    ...
    def call_llm(self, ctx, payload):
        ...
        # Per-agent reasoning effort — resolved into effectiveAgentConfig.llm by
        # resolve_llm_metadata (agentConfig.reasoningEffort); stamped onto the
    ...
    def agent_workflow(self, ctx, message: dict):
        ...
        # Per-agent reasoning effort from the same per-turn snapshot (see
        # resolve_llm_metadata); adapters map it per provider. Restored with
        # the component on workflow_terminal.
        previous_reasoning_effort = getattr(self.llm, "_reasoning_effort", None)
