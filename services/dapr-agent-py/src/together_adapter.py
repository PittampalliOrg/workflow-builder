"""Together AI chat-completions adapter for DaprChatClient.

Together exposes an OpenAI-compatible chat completions API. Keep Together
models on the same durable-agent contract as NVIDIA and Foundry: normal calls
return LLMChatResponse and structured calls return the requested Pydantic model.
"""
...
COMPONENT_MODEL_MAP: dict[str, str] = {
    "llm-together-glm-51": "zai-org/GLM-5.1",
    "llm-together-qwen3-coder-480b": "Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8",
    "llm-together-deepseek-v4-pro": "deepseek-ai/DeepSeek-V4-Pro",
}


def _is_together_component(component: str) -> bool:
    return component in COMPONENT_MODEL_MAP or "together" in str(component or "").lower()


def _get_together_model(component: str) -> str:
    return COMPONENT_MODEL_MAP.get(
        component,
        os.environ.get("TOGETHER_DEFAULT_MODEL", "zai-org/GLM-5.1"),
    )
...
def _normalize_messages_for_together(
    ...
    return ensure_chat_completions_history(messages, provider="together")
...
def _convert_tools_for_together_chat(
    tools: list[Any] | None,
...
def _extract_together_response(
...
def _auth_headers() -> tuple[dict[str, str], str]:
    ...
    if not api_key:
        raise RuntimeError("No Together AI authentication configured. Set TOGETHER_API_KEY.")
...
def _retry_after_seconds(exc: HTTPError) -> float:
    ...
    return max(
        0.0,
        float(os.environ.get("TOGETHER_RATE_LIMIT_BACKOFF_SECONDS", "65")),
    )


def _rate_limit_max_retries() -> int:
    return max(0, int(os.environ.get("TOGETHER_RATE_LIMIT_MAX_RETRIES", "3")))
...
def _publish_llm_usage(
    *,
    ...
    try:
        ...
        logger.debug("[session-event] together llm_usage emit failed: %s", exc)


def _call_together_chat(
    component: str,
    messages: list[dict[str, Any]],
    ...
    model = _get_together_model(component)
    converted_tools = _convert_tools_for_together_chat(tools)
    ...
    try:
        ...
        llm_span = start_llm_request_span(
            model,
            fast_mode=False,
            query_source="dapr_agent_py.together_adapter",
            system_prompt="\n\n".join(
        ...
        logger.warning("[telemetry] llm_request (together) start failed: %s", exc)
    ...
    timeout = int(os.environ.get("TOGETHER_TIMEOUT_SECONDS", "600"))
    output_cap = max_tokens or int(os.environ.get("TOGETHER_MAX_TOKENS", "1024"))
    ...
    if converted_tools:
        ...
        if tool_choice in (None, "", "auto"):
            ...
            logger.warning(
                "[together-chat] ignoring unsupported forced tool_choice=%r",
                tool_choice,
            )
            request_body["tool_choice"] = "auto"
    ...
    logger.info(
        "[together-chat] Calling %s with %d messages, %d tools, auth=%s",
        model,
        len(messages),
    ...
    try:
        while True:
            req = _make_together_request(url, request_body, headers)
            try:
                ...
                if exc.code == 429 and attempt < rate_limit_retries:
                    ...
                    attempt += 1
                    logger.warning(
                        "[together-chat] 429 rate limit from %s; retrying in %.1fs "
                        "(attempt %d/%d): %s",
                        model,
                        delay,
                        attempt,
                    ...
                    continue
                raise RuntimeError(
                    f"Together AI Chat API failed ({exc.code}): {detail}"
                ) from exc
    ...
    content, tool_calls, finish_reason, reasoning_content = _extract_together_response(data)
    usage = data.get("usage") if isinstance(data.get("usage"), dict) else {}
    duration_ms = (time.monotonic() - llm_start) * 1000.0
    if response_format is not None and not content.strip():
        error = (
            "Together AI Chat API returned empty assistant content for "
    ...
    if llm_span is not None:
        try:
            ...
            logger.warning("[telemetry] llm_request (together) end failed: %s", exc)
    ...
    try:
        ...
        logger.debug("[genai-attrs] together span enrichment failed: %s", _attr_exc)

    return result


def patch_for_together(llm_client: Any) -> None:
    """Patch DaprChatClient to use Together's OpenAI-compatible chat endpoint."""

    from dapr_agents.llm.dapr.chat import DaprChatClient

    if getattr(DaprChatClient, "_together_patched", False):
        return
    ...
    def patched_generate(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if component and _is_together_component(component):
    ...
    DaprChatClient._together_patched = True
    logger.info("[together-chat] Patched DaprChatClient class for Together direct calls")
