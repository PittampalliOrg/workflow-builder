"""Prompt and constants for the TodoList tool.

Aligned with kimi-code v2's TodoList tool
(packages/agent-core-v2/src/session/todo/tools/todo-list.md): `todos`
replaces the full list, omitting `todos` queries the current list, and
`todos: []` clears it.
"""
...
def get_todo_list_description() -> str:
    return """Use this tool to maintain a structured TODO list as you work through a multi-step task. Use it proactively and often when progress tracking helps the current work. This is especially useful in long-running investigations and implementation tasks with several tool calls.
