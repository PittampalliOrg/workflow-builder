...
Ported from claude-code-src/main/services/compact/prompt.ts. The exact
wording matters — the model is trained to produce <analysis>/<summary>
blocks in response to this framing, and `format_compact_summary()`
strips the drafting scratchpad before storing the summary back in
history.
"""
...
def get_compact_prompt(custom_instructions: str | None = None) -> str:
    """Return the full summarization prompt, optionally with user custom instructions.

    Mirrors `getCompactPrompt()` in prompt.ts:293.
    """
...
def format_compact_summary(summary: str) -> str:
    """Strip the <analysis> drafting scratchpad and format <summary> section.

    Mirrors `formatCompactSummary()` in prompt.ts:311 exactly: keep first
    match semantics, replace only the first <analysis> and first <summary>
    tag pair, collapse runs of blank lines.
    """
    ...
    formatted = _MULTI_NEWLINE_RE.sub("\n\n", formatted)
