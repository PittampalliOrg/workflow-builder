...
def accept_team_mailbox_delivery(
    ...
    *,
    ...
    """Return only not-yet-consumed events from a team mailbox delivery.

    Dapr can durably accept a RaiseEvent even when the HTTP caller loses its
    response. Retrying the same claim, or reclaiming it together with newer
    rows, must not start a second model turn for an already-consumed event.
    Receipt ids live in workflow state, so replay rebuilds them deterministically.
    """
    ...
    if (
        ...
        # Malformed optional metadata must never discard user input.
        return events
