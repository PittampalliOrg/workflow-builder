...
def test_kimi_auth_headers_accept_moonshot_fallback(monkeypatch) -> None:
    monkeypatch.delenv("KIMI_API_KEY", raising=False)
    monkeypatch.setenv("MOONSHOT_API_KEY", "moonshot-test")

    headers, auth_mode = adapter._auth_headers()

    assert headers["Authorization"] == "Bearer moonshot-test"
    assert auth_mode == "moonshot-api-key"


def test_kimi_chat_uses_openai_compatible_endpoint_key_and_thinking(monkeypatch) -> None:
    bodies: list[dict] = []
    ...
    assert bodies[0]["model"] == "kimi-k2.6"
    ...
    assert bodies[0]["prompt_cache_key"] == "agent_1:3"
    assert bodies[0]["stream"] is False
    assert result["content"] == "ok"
    assert result["metadata"]["provider"] == "kimi-chat"


def test_kimi_chat_selects_k25_model(monkeypatch) -> None:
    bodies: list[dict] = []
    ...
    assert bodies[0]["model"] == "kimi-k2.5"
...
def test_kimi_structured_output_uses_json_object_and_disables_thinking(
    monkeypatch,
) -> None:
    bodies: list[dict] = []
