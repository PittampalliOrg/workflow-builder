 ...
 *
 * Pre-fetches credentials from Dapr secret store before routing to functions.
 * This centralizes credential management and avoids each function needing
...
const INTEGRATION_CONFIG_TO_ENV: Record<string, Record<string, string>> = {
	database: { databaseUrl: "DATABASE_URL" },
...
async function fetchConnectionCredentials(
	...
	try {
		...
		// If the caller's integrationType doesn't have a mapping (e.g. system/* actions),
		// fall back to the decrypted connection's actual pieceName.
		const primaryCreds = mapConnectionValueToEnvVars(
			data.value,
			integrationType,
		);
		const creds =
			Object.keys(primaryCreds).length > 0
				? primaryCreds
				: mapConnectionValueToEnvVars(data.value, data.pieceName);
		console.log(
