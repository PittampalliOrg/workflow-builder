...
const OTEL_CONSOLE_BRIDGE_KEY =
  "__workflow_builder_function_router_console_otel__";
type OtelLogger = {
  emit(record: {
    severityText: string;
    body: string;
...
function requestField(request: unknown, keys: string[]): string {
  ...
  const getHeader = record.getHeader;
  if (typeof getHeader === "function") {
    for (const key of keys) {
      try {
        const value = getHeader.call(request, key);
        if (typeof value === "string" && value.trim()) return value.trim();
...
function ignoreContentTracedOutgoingRequest(request: unknown): boolean {
  const path = requestField(request, ["path", "pathname"]);
  const host = requestField(request, [
    "host",
    "hostname",
    "Host",
    "server.address",
  ...
  if (path === "/execute" || path.startsWith("/api/workspaces/")) {
    return (
      host.includes("code-runtime") ||
      host.includes("fn-") ||
      host.includes("openshell-agent-runtime") ||
      host.includes("sandbox-execution-api") ||
      host.includes("dapr-swe")
    );
  }
  return false;
}
    ...
    instrumentations: [
      getNodeAutoInstrumentations({
        ...
        "@opentelemetry/instrumentation-http": {
          ignoreOutgoingRequestHook: ignoreContentTracedOutgoingRequest,
        },
      }),
    ],
  });
...
export function otelLogMixin(): Record<string, string> {
  ...
  const sessionId =
    baggage?.getEntry("session.id")?.value ??
    baggage?.getEntry("sessionId")?.value;
