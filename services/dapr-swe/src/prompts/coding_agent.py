"""Full system prompt for the single CodingAgent, ported from open-swe."""

from __future__ import annotations

# Untrusted comment tag — same value as open-swe uses.
...
def construct_system_prompt(
    ...
    """Build the full CodingAgent system prompt with runtime values.

    Parameters
    ----------
    working_dir:
        Sandbox working directory path.
    issue_context:
        Dict with keys: owner, repo, issue_number, linear_project_id,
        linear_issue_number, agents_md (all optional).
    """
    linear_project_id = issue_context.get("linear_project_id", "") or "<PROJECT_ID>"
    linear_issue_number = issue_context.get("linear_issue_number", "") or "<ISSUE_NUMBER>"
