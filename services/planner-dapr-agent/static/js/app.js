...
class PlannerAgent {
    ...
    init() {
        ...
        this.elements.approveBtn.addEventListener('click', () => this.approveWorkflow(true));
        this.elements.rejectBtn.addEventListener('click', () => this.approveWorkflow(false));
    ...
    async fetchWorkflowState(workflowId) {
        try {
            ...
            if (data.phase === 'awaiting_approval' || data.status === 'AWAITING_APPROVAL') {
    ...
    handleEvent(data) {
        ...
        if (data.phase === 'awaiting_approval' || data.type === 'approval_required') {
            this.showApprovalButtons();
        }

        // Hide approval buttons if workflow progressed past approval
        if (data.phase === 'execution' || data.phase === 'testing' || data.phase === 'completed') {
    ...
    addEvent(event) {
        ...
        const message = event.message || event.data?.message || this.formatEventMessage(event);
    ...
    getStatusClass(status) {
        if (status.includes('running') || status.includes('execution') || status.includes('planning') || status.includes('testing')) {
            return 'running';
        }
    ...
    async approveWorkflow(approved) {
        ...
        try {
            ...
            this.addEvent({
                ...
                message: approved ? 'Plan approved - continuing to execution' : 'Plan rejected',
                timestamp: new Date().toISOString()
            });
