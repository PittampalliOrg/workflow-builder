...
# When using secretstores.kubernetes, we fetch from this K8s secret
K8S_SECRET_NAME = os.environ.get("K8S_SECRET_NAME", "ai-chatbot-secrets")

# Secret mappings from application env var names to Azure Key Vault secret names
# Azure Key Vault uses lowercase hyphenated names (e.g., "openai-api-key")
SECRET_MAPPINGS: dict[str, str] = {
    # AI Providers - values are the Azure Key Vault secret names
...
async def initialize_config_and_secrets() -> None:
    ...
    It will:
    1. Check if Dapr sidecar is available (with retries)
    ...
    async with _initialization_lock:
        ...
        # Wait for Dapr sidecar with retries (handles container startup race condition)
        max_retries = int(os.environ.get("DAPR_STARTUP_RETRIES", "10"))
        retry_delay = float(os.environ.get("DAPR_STARTUP_DELAY", "1.0"))

        for attempt in range(max_retries):
            try:
                _dapr_available = await client.is_available()
                if _dapr_available:
                    logger.info(f"[ConfigProvider] Dapr sidecar available (attempt {attempt + 1})")
                    break
            except Exception:
                pass

            if attempt < max_retries - 1:
                logger.info(f"[ConfigProvider] Waiting for Dapr sidecar (attempt {attempt + 1}/{max_retries})...")
                await asyncio.sleep(retry_delay)

        if not _dapr_available:
            logger.info("[ConfigProvider] Dapr sidecar not available after retries, using environment variables")
...
async def _load_secrets_from_dapr() -> None:
    ...
    For Kubernetes secretstores, fetches all keys from K8S_SECRET_NAME.
    For Azure Key Vault, fetches each secret individually.
    """
    global _secret_cache

    # Try to fetch all secrets from Kubernetes secret first
    try:
        # For Kubernetes secretstores, fetching by secret name returns all keys
        secrets = await client.get_secret(SECRET_STORE, K8S_SECRET_NAME)
        if secrets and isinstance(secrets, dict):
            # Kubernetes returns {key: value, key: value, ...}
            for env_key, secret_key in SECRET_MAPPINGS.items():
                if secret_key in secrets:
                    _secret_cache[env_key] = secrets[secret_key]
            logger.info(f"[ConfigProvider] Loaded secrets from Kubernetes secret {K8S_SECRET_NAME}")
            return
    except Exception as e:
        logger.debug(f"[ConfigProvider] Kubernetes secret fetch failed, trying individual secrets: {e}")

    # Fallback: try individual secrets (for Azure Key Vault or other stores)
