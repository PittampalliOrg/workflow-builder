-- Persist the exact cross-system teammate launch handoff. A launch remains in
-- `starting` until its published session/runtime generation is proven; a stale
-- invalid launch is first fenced for cleanup so parallel reconcilers cannot
-- race a promotion. Revival predecessor fields make compensation exact.
