	...
	try {
		...
		await db
			...
			.set({
				...
				errorStackTrace: orchestratorStatus.stackTrace ?? null,
		...
		return NextResponse.json({
			...
			workflowVersion: orchestratorStatus.workflowVersion ?? null,
			workflowNameVersioned: orchestratorStatus.workflowNameVersioned ?? null,
			phase,
			progress,
			message,
			...
			error: errorMessage,
			stackTrace: orchestratorStatus.stackTrace ?? null,
		});
	} catch (error) {
