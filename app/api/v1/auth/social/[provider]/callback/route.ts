...
export async function GET(
	...
	try {
		...
		appUrl = await resolveSocialAppUrl(request);
		...
		if (!(code && state)) {
			return NextResponse.redirect(
				`${appUrl}/sign-in?error=Missing+code+or+state+parameter`,
			);
		}
		...
		if (!storedState || storedState !== state) {
			return NextResponse.redirect(
				`${appUrl}/sign-in?error=Invalid+OAuth+state`,
			);
		}
		...
		if (provider === "github") {
			...
			return NextResponse.redirect(
				`${appUrl}/sign-in?error=Unsupported+provider`,
			);
		}
		...
		return NextResponse.redirect(
			`${appUrl}/sign-in?error=${encodeURIComponent(errorMessage)}`,
		);
	}
}
