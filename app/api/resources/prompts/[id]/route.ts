...
import {
	deleteResourcePromptById,
	getResourcePromptByIdForRead,
	updateResourcePromptById,
...
export async function GET(request: Request, { params }: RouteParams) {
	try {
		...
		const row = await getResourcePromptByIdForRead({
			id,
...
export async function PATCH(request: Request, { params }: RouteParams) {
	try {
		...
		const { id } = await params;
		const updated = await updateResourcePromptById({
			id,
			userId: session.user.id,
			projectId: session.user.projectId,
			input: parsed.data,
...
export async function DELETE(request: Request, { params }: RouteParams) {
	try {
		...
		const { id } = await params;
		const ok = await deleteResourcePromptById({
			id,
			userId: session.user.id,
			projectId: session.user.projectId,
		});
