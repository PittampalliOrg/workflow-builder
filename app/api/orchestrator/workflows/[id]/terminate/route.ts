	...
	try {
		...
		await db
			...
			.set({
				status: "cancelled",
				phase: "failed",
				error: reason || "Workflow terminated",
		...
		console.error("[Orchestrator API] Error terminating workflow:", error);
		return NextResponse.json(
			{ error: "Failed to terminate workflow", message: errorMessage },
			{ status: 500 },
		);
	}
}
