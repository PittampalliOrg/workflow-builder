...
export async function POST(
	...
	try {
		...
		const result = await genericOrchestratorClient.pauseWorkflow(
			orchestratorUrl,
		...
		console.error("[Orchestrator API] Error pausing workflow:", error);
		return NextResponse.json(
			{ error: "Failed to pause workflow", message: errorMessage },
			{ status: 500 },
		);
	}
}
