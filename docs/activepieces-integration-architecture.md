...
| Idempotency | `piece_execution` table; `idempotency_key = workflowId:dbExecutionId:taskName` (stable across retries AND replay). `/execute` gates with an upsert: a **completed** row returns the cached result (`deduped: true`) — the common duplicate-side-effect case (action succeeded, response lost, activity retried) produces exactly one side effect; a mid-flight crash re-executes (at-least-once; true exactly-once over external APIs is impossible). 'paused' rows let RESUME proceed. Gate DB failures are fail-closed (returned as retryable). Per-node `idempotent: true` opt-out skips the gate. The table doubles as the per-execution piece audit trail. |
...
| Payload ceiling | The 16 MiB Dapr body cap binds on the orchestrator↔router legs. `/execute` offloads results > 4 MiB (env `MAX_INLINE_RESULT_BYTES`) into the `piece_execution` row (already stored for idempotency — no double persistence) and returns `{artifactRef: {kind: "piece_execution", idempotencyKey}, preview, truncated: true}`; full payload readable via BFF `GET /api/internal/piece-executions/[idempotencyKey]`. |
