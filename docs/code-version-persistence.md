...
| **Durable, re-accessible *code*** | ❌ | On the last completed run, **115/115 checkpoints had `remoteStatus=skipped`** — commits were created locally (`afterSha` set) but never pushed. They lived only in the JuiceFS `/sandbox/work` workspace, which is later GC'd (`sandbox-gc` / `lifecycle-terminal-reaper`). So the *actual code* of a past version is gone; only the diff *blobs* survive. |
