...
- ✅ **`lifecycle-terminal-reaper` CronJob** → `POST /api/internal/lifecycle/reap-terminal` — reconciles DB rows stuck non-terminal vs terminal/gone Dapr instances, purges orphans, and finalizes stop-requested rows. (Post-#69 it reconciles the terminal/gone divergence **even during benchmark activity** — the per-row terminal/gone guard is the safety; post-#79 only its *aged stuck-execution* pass defers to an execution owned by a **still-active** coordinator run. See Part 7 §P1/§P6.)
...
- **Phase 4 — GitOps safety nets (stacks #2523).** ✅ unified `stateRetentionPolicy = 168h` (parent + per-session children); `workflow-builder-sandbox-gc` CronJob; `lifecycle-terminal-reaper` CronJob → `POST /api/internal/lifecycle/reap-terminal` (reconciles even during benchmark activity post-#69; only the aged-stuck pass defers to an active-coordinator-owned exec post-#79). Via the cluster-update/GitOps flow.
...
- **Reaper × active coordinator (#5).** The reaper's *aged stuck-execution* pass now skips an execution owned by a **still-active** benchmark/eval run (`reaper.ts::ownedByActiveCoordinatorRun`), so it can't purge an instance the coordinator is about to re-drive; it releases once the owning run is terminal. (The terminal/gone reconcile + stop-requested priority passes still run regardless — that's #69.)
- **Null-linkage fan-out (#6).** `resolvers.ts::agentTargetForSession` only synthesizes the deterministic per-session app-id when there's **per-session-sandbox evidence** (`runtimeSandboxName` set); an unstarted/pool-hosted session with no linkage resolves to *unresolved* → the cascade reports "stopping" + the reaper retries, instead of terminating a nonexistent instance and falsely declaring the agent closed.
