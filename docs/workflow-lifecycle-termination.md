...
The fix is **BFF-only** (zero orchestrator/agent risk): treat the wedge as **DB-state cleanup**, since the agent is already stopped. `confirmDurableStop` now splits `parentClosed`/`agentClosed`; when the parent is still RUNNING but **every** agent-runtime child is terminal/gone, there is ≥1 child + ≥1 parent instance, and **`LIFECYCLE_WEDGE_FINALIZE_GRACE_SECONDS`** (default **180s**) has elapsed since `stop_requested_at`, it force-deletes the wedged parent's durable state rows directly (`purgeStateRows` — the same mechanism `mode:"reset"` uses), best-effort `purgeParent`, then reaps + finalizes. The grace is the safety: a parent that's merely slow-to-terminate or legitimately progressing a later same-task-hub node terminates normally within it and never hits this path. The decision is the pure, unit-tested `shouldForceFinalizeCrossAppWedge` (`cascade.ts`). The reaper routes Stop-requested rows whose parent isn't yet terminal through `confirmDurableStop`, so it converges even if the tab closed. `call_child_workflow` is **untouched**.
