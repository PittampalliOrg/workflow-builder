...
Runtime caveat: the Gemini-backed `agy-cli` compatibility slot treats legacy `"auto"` like an explicit-only mode for implicit project fan-in. It still resolves explicitly attached project MCP references, and `"project"` still intentionally inherits every enabled workspace MCP server. This avoids presenting globally available MCP servers as configured `agy-cli` servers when an agent did not explicitly attach them.
