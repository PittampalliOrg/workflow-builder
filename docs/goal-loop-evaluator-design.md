...
>
> UPDATE (2026-06-18): the independent-evaluator question is answered + implemented in **`docs/generator-critic-multi-agent.md`** — a workflow-native generator+critic evaluator-optimizer (SW 1.0 `for`/`while` loop): the generator has no `update_goal`, a separate critic agent judges a rubric, and **objective criteria gate on a deterministic `workspace/command` ground-truth check ANDed with the critic** (proven necessary after the LLM critic was observed hallucinating a pass on an objective rule). Phases 1–3 verified on ryzen; see that doc for the SSOT.
