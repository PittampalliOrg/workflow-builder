...
function hasSessionCookies(): boolean {
	...
	return (
		cookie.includes("wb_access_token=") || cookie.includes("wb_refresh_token=")
	);
}
...
export function seedSession(session: SessionData) {
	const nextUserId = session?.user.id ?? null;
	const currentUserId = globalSession?.user.id ?? null;
	if (!globalLoading && currentUserId === nextUserId) {
		return;
	}
