...
export type CodingWorkflowBackend = "openshell-langgraph";
...
export function buildCodingWorkflow(
	params: CodingWorkflowParams,
): WorkflowTemplate {
	const { repoUrl = "", prompt = "", maxTurns = 50 } = params;
	const actionPrefix = "openshell-langgraph-observable";
	...
	const edges: TemplateEdge[] = [
		...
		{
			id: "e-review-validate",
			source: "review-changes",
			target: "browser-validate",
		},
	];
