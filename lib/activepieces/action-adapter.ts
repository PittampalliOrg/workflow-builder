...
export type ApIntegration = {
	...
	pieceName: IntegrationDefinition["pieceName"];
	logoUrl: string;
	actions: IntegrationDefinition["actions"];
};
...
function convertApActionToWbAction(
	...
	action: ApAction,
	displayName: string,
): IntegrationDefinition["actions"][number] {
...
export function convertApPieceToIntegration(
	...
	// Normalize to canonical short slug (e.g. google_sheets -> google-sheets).
	const pieceName = normalizePieceName(piece.name);
	...
	const actions: IntegrationDefinition["actions"] = [];
