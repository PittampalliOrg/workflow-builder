...
- Prefer configuring repositoryOwner, repositoryRepo, and repositoryBranch directly on workspace/clone so the repository is chosen in the action config, not asked again at run time.
- Only add manual trigger inputs for repository selection when the user explicitly wants the repository or branch to vary per run.
