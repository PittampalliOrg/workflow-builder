...
<div class="flex h-full flex-col">
	<header class="flex h-12 items-center justify-between border-b border-border px-6">
		<div class="flex min-w-0 items-center gap-3">
			<Button variant="ghost" size="icon" href="/admin/instances/names">
				<ArrowLeft size={15} />
			</Button>
			<div class="min-w-0">
		...
		<div class="flex items-center gap-2">
			<Button variant="outline" size="sm" href="/workspaces/default/workflows">
	...
	<div class="flex-1 overflow-auto p-6">
		<div class="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
			<a class="hover:text-foreground" href="/admin/instances/names">Workflows</a>
			<span>/</span>
								...
								<TableRow class="transition-colors hover:bg-muted/30">
									...
									<TableCell class="p-0 font-mono text-xs">
										<a
											...
											href={`/admin/instances/${encodeURIComponent(row.instanceId)}`}
										>
											{row.instanceId}
										</a>
									</TableCell>
									<TableCell class="p-0">
										<a class="block px-4 py-3" href={`/admin/instances/${encodeURIComponent(row.instanceId)}`}>
											<Badge variant={statusVariant(currentStatus)}>{currentStatus}</Badge>
										</a>
									</TableCell>
									<TableCell class="p-0 text-sm text-muted-foreground">
										<a class="block px-4 py-3" href={`/admin/instances/${encodeURIComponent(row.instanceId)}`}>{row.dapr?.workflowVersion ?? 'Non-versioned'}</a>
									</TableCell>
									<TableCell class="p-0 text-sm text-muted-foreground">
										<a class="block px-4 py-3" href={`/admin/instances/${encodeURIComponent(row.instanceId)}`}>{formatTime(row.startedAt)}</a>
									</TableCell>
									<TableCell class="p-0 text-sm text-muted-foreground">
										<a class="block px-4 py-3" href={`/admin/instances/${encodeURIComponent(row.instanceId)}`}>{formatDuration(row.durationMs)}</a>
									</TableCell>
								</TableRow>
							{/each}
						{/if}
