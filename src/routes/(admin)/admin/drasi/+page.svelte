<script lang="ts">
	...
	// Graph item whose canvas element should regain focus after the detail
	// sheet closes (keyboard parity with pointer dismissal).
	let sheetReturnTarget: { kind: "node" | "edge"; id: string } | null = null;

	function handleSelect(next: DrasiSelection | null) {
		if (next !== null) sheetReturnTarget = { kind: next.kind, id: next.id };
		selection = next;
		sheetOpen = next !== null;
	}

	function inspectNode(nodeId: string) {
		sheetReturnTarget = { kind: "node", id: nodeId };
		selection = { kind: "node", id: nodeId };
		sheetOpen = true;
	}

	function focusGraphItem(target: { kind: "node" | "edge"; id: string }) {
		// Always resolve the element fresh: XYFlow may re-create the wrapper
		// while the sheet is open or during close (scroll-lock release
		// re-measures the viewport), so a captured element reference can be
		// detached by the time it receives focus.
		const selector =
			target.kind === "node"
				? `.svelte-flow__node[data-id="${CSS.escape(target.id)}"]`
				: `.svelte-flow__edge[data-id="${CSS.escape(target.id)}"]`;
		document.querySelector<HTMLElement>(selector)?.focus({ preventScroll: true });
	}

	// Return focus to the triggering graph node/edge when the detail sheet
	// closes. Driven by the bound `open` true→false transition so it runs
	// after the dialog's own teardown; the post-tick frame focus plus a short
	// bounded re-assert tail (only while focus is stranded on <body>) cannot
	// be overwritten by the dialog's cleanup and never fights a user who
	// deliberately moved focus elsewhere.
	$effect(() => {
		if (sheetOpen) return;
		const target = sheetReturnTarget;
		if (!target) return;
		sheetReturnTarget = null;
		void tick().then(() => {
			if (sheetOpen) return;
			focusGraphItem(target);
			requestAnimationFrame(() => {
				// Re-assert on a bounded tail: XYFlow can re-create the wrapper a
				// few frames after close (viewport re-measure when the dialog's
				// scroll-lock releases), which strands a previously restored
				// focus back on <body>. Poll briefly and only refocus while
				// focus is actually stranded — never steal it from a user who
				// deliberately moved on.
				let attempts = 0;
				const reassert = () => {
					if (sheetOpen) return;
					attempts += 1;
					if (document.activeElement === document.body) focusGraphItem(target);
					if (attempts < 12) window.setTimeout(reassert, 125);
				};
				reassert();
			});
		});
