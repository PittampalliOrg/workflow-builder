<script lang="ts">
	...
	function workflowHref(appId: string, name: string): string {
		return `/workflow-ops/names/${encodeURIComponent(appId)}/${encodeURIComponent(name)}`;
	}
...
<div class="flex h-full flex-col">
	<header class="flex h-12 items-center justify-between border-b border-border px-6">
		...
		<div class="flex items-center gap-2">
			<Button variant="outline" size="sm" href="/workflow-ops/executions">All workflow executions</Button>
	...
	<div class="flex-1 overflow-auto p-6">
		<div class="mb-5 flex border-b border-border">
			<a href="/workflow-ops/names" class="border-b-2 border-primary px-4 py-2 text-sm font-medium">Workflows</a>
			<a href="/workflow-ops/executions" class="px-4 py-2 text-sm text-muted-foreground hover:text-foreground">All workflow executions</a>
		</div>
