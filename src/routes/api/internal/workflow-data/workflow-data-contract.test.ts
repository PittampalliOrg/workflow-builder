...
const mocks = vi.hoisted(() => {
  ...
  const workflowExecutionRuntimeHosts = { requestReap: vi.fn() };
  return {
    workflowData,
...
vi.mock("$lib/server/internal-auth", () => ({
  ...
  requireInternalOrPreviewControlRead: mocks.requireInternal,
}));

import { GET as getWorkflow } from "./workflows/[workflowRef]/+server";
import {
  GET as getStaleExecutions,
  POST as postExecution,
} from "./executions/+server";
import {
  GET as getExecution,
  PATCH as patchExecution,
} from "./executions/[executionId]/+server";
...
import {
  GET as getPlanArtifact,
  PATCH as patchPlanArtifact,
} from "./plan-artifacts/[artifactRef]/+server";
...
const CONTRACT: Record<
  string,
  { handler: RouteHandler; arrange: (f: ContractFixture) => void }
> = {
  "get-workflow": {
    handler: getWorkflow,
    arrange: (f) =>
      mocks.workflowData.getWorkflowByRef.mockResolvedValue(
        f.responseBody.workflow,
      ),
  },
  "get-execution": {
    ...
    arrange: (f) =>
      mocks.workflowData.getExecutionById.mockResolvedValue(
        f.responseBody.execution,
      ),
  },
  "read-model-ready": {
    handler: getReadModelReady,
    arrange: () =>
      mocks.workflowData.assertExecutionReadModelReady.mockResolvedValue(
        undefined,
      ),
  },
  "get-execution-by-instance": {
    ...
    arrange: (f) =>
      mocks.workflowData.getExecutionByDaprInstanceId.mockResolvedValue(
        f.responseBody.execution,
      ),
  },
  "create-execution": {
    ...
    arrange: (f) =>
      mocks.workflowData.createWorkflowExecution.mockResolvedValue(
        f.responseBody,
      ),
  },
  "get-live-execution-instance": {
    ...
    arrange: (f) =>
      mocks.workflowData.getLiveExecutionInstance.mockResolvedValue(
        f.responseBody.instance,
      ),
  },
  "attach-scheduler-instance": {
    handler: postSchedulerInstance,
    arrange: () =>
      mocks.workflowData.attachExecutionSchedulerInstance.mockResolvedValue(
        undefined,
      ),
  },
  "mark-start-failed": {
    ...
    arrange: () =>
      mocks.workflowData.markExecutionStartFailed.mockResolvedValue(undefined),
  },
  "list-stale-executions": {
    handler: getStaleExecutions,
    arrange: (f) =>
      mocks.workflowData.listStaleRunningExecutions.mockResolvedValue(
        f.responseBody.executions,
      ),
  },
  "patch-execution": {
  ...
  "append-execution-log": {
    ...
    arrange: (f) => {
      ...
      mocks.workflowData.appendExecutionLog.mockResolvedValue(
        f.responseBody.log,
      );
    },
  },
  "update-execution-log": {
    ...
    arrange: (f) => {
      ...
      mocks.workflowData.updateExecutionLog.mockResolvedValue(
        f.responseBody.log,
      );
    },
  },
  "upsert-workflow-artifact": {
    ...
    arrange: () =>
      mocks.workflowData.upsertWorkflowArtifact.mockResolvedValue(undefined),
  },
  ...
  "resolve-mcp-config": {
    ...
    arrange: (f) =>
      mocks.workflowData.resolveMcpConfig.mockResolvedValue(f.responseBody),
  },
  "schedule-agent-run": {
    handler: postAgentRun,
    arrange: (f) =>
      mocks.workflowData.upsertScheduledAgentRun.mockResolvedValue(
        withoutOk(f.responseBody),
      ),
  },
  "update-agent-run": {
    ...
    arrange: (f) =>
      mocks.workflowData.updateAgentRunLifecycle.mockResolvedValue(
        withoutOk(f.responseBody),
      ),
  },
  "upsert-plan-artifact": {
    ...
    arrange: (f) =>
      mocks.workflowData.upsertPlanArtifact.mockResolvedValue(
        withoutOk(f.responseBody),
      ),
  },
  "update-plan-artifact": {
    ...
    arrange: (f) =>
      mocks.workflowData.updatePlanArtifactStatus.mockResolvedValue(
        withoutOk(f.responseBody),
      ),
  },
  "get-plan-artifact": {
    ...
    arrange: (f) =>
      mocks.workflowData.getPlanArtifact.mockResolvedValue(
        f.responseBody.artifact,
      ),
  },
  "get-trace-targets": {
    handler: getTraceTargets,
    arrange: (f) =>
      mocks.workflowData.getTraceTargetsForExecution.mockResolvedValue(
        f.responseBody.targets,
      ),
  },
  "upsert-trace-lineage": {
    handler: postTraceLineage,
    arrange: (f) =>
      mocks.workflowData.upsertTraceLineageLinks.mockResolvedValue(
        withoutOk(f.responseBody),
      ),
  },
};
