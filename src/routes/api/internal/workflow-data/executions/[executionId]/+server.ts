...
export const GET: RequestHandler = async ({ params, request }) => {
	...
	const execution = await getApplicationAdapters().workflowData.getExecutionById(executionId);
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		return error(400, err instanceof Error ? err.message : "invalid execution patch");
	}
	if (Object.keys(patch).length === 0) return error(400, "no supported execution fields supplied");
