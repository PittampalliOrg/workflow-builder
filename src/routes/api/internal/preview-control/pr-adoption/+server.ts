...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (
		...
		bodyIdentity.environmentPlatformRevision !==
			identity.environmentPlatformRevision ||
		bodyIdentity.environmentSourceRevision !==
		...
		return json(
			{
				ok: false,
				error: 'PR adoption authority does not match this preview',
			},
			{ status: 409 },
		);
	}
	...
	try {
		stagedServices = JSON.parse(
			env.PREVIEW_ENVIRONMENT_SERVICES_JSON ?? '',
		);
	} catch {
		return json(
			{ ok: false, error: 'preview service authority is unavailable' },
			{ status: 503 },
		);
	}
	if (
		...
		JSON.stringify([...services].sort()) !==
			JSON.stringify([...stagedServices].sort())
	) {
		return json(
			{
				ok: false,
	...
	if (
		...
		return json(
			{ ok: false, error: 'PR adoption options are invalid' },
			{ status: 400 },
		);
	}
	try {
		...
		if (cause instanceof RetryableDevPreviewActivationError) {
			return json(
				{
					retryable: true,
					complete: false,
