...
export const POST: RequestHandler = async ({ request }) => {
  ...
  >;
  const action =
    body.action === "up" || body.action === "down" ? body.action : null;
  const prNumber = Number(body.prNumber);
  if (!action || !Number.isInteger(prNumber) || prNumber <= 0) {
    return json(
      {
        error:
          "action ('up'|'down') and a positive integer prNumber are required",
      },
      { status: 400 },
    );
  }
  const allowed =
    action === "down"
      ? new Set(["action", "prNumber"])
      : new Set(["action", "prNumber", "headSha", "verify"]);
  const unsupported = Object.keys(body).filter((key) => !allowed.has(key));
  if (unsupported.length > 0) {
    return json(
      {
        error: `unsupported PR preview fields: ${unsupported.sort().join(", ")}; GitHub and the server catalog own deployment authority`,
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (!headSha) {
      return json(
        { error: "headSha is required for action 'up'" },
        { status: 400 },
      );
    }
    ...
    console.error("[pr-preview] request failed", error);
    return json({ error: "PR preview request failed" }, { status: 502 });
  }
};
