...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!anthropicKey && !openaiKey) {
		return error(503, 'No AI API key configured (ANTHROPIC_API_KEY or OPENAI_API_KEY)');
	}
	...
	try {
		...
		return error(500, `AI generation failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
	}
};
