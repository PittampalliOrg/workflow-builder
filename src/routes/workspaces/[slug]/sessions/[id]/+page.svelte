<script lang="ts">
	...
	async function refreshRuntimeFlags() {
		try {
			...
			runtimeFlags = {
				...
				nativeGoalAvailable: body.nativeGoalAvailable === true,
	...
	const nativeGoalAvailable = $derived(runtimeFlags?.nativeGoalAvailable === true);
	...
	>
		...
		>
			{#if session}
				...
				<SessionGoalBadge {sessionId} nativeAvailable={nativeGoalAvailable} />
