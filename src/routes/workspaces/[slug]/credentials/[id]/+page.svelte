...
<div class="h-full overflow-y-auto flex flex-col max-w-5xl mx-auto w-full p-6 gap-6">
	<nav class="flex items-center gap-1 text-xs text-muted-foreground">
		...
		<a href="/workspaces/{slug}/credentials" class="hover:text-foreground">Credentials</a>
		<span class="text-muted-foreground/60">/</span>
		<span class="text-foreground truncate max-w-[260px]">{vault?.name ?? 'Loading…'}</span>
	</nav>
	<div class="flex items-center gap-2 flex-wrap">
		<Button variant="ghost" size="sm" onclick={() => goto(`/workspaces/${slug}/credentials`)}>
