...
import { CLICKHOUSE_DB, escapeClickHouseString, queryClickHouse } from '$lib/server/otel/clickhouse';
...
import type { CapacityBusinessWorkSummary, CapacityOverviewSummary } from '$lib/types/capacity';
...
export const getCapacityTrends = query(
	...
	try {
		const [requestedRows, allocRows, admittedRows, pendingRows, reservingRows, latencyRows] =
			await Promise.all([
		...
		const utilizationPctByResource: Record<string, CapacityPsiTrendPoint[]> = {};
		...
		const toSeries = (rows: Record<string, unknown>[]): CapacityPsiTrendPoint[] =>
			rows.map((r) => ({ t: new Date(String(r.bucket)).toISOString(), value: Number(r.v) || 0 }));
		...
		const hasData =
			...
			admitted.length + pending.length + reserving.length + latencyAvgMs.length > 0;
...
// ---------------------------------------------------------------------------
export type CapacityOwnerTimelinePoint = { t: string; values: Record<string, number>; other: number };
...
export const getCapacityOwnerTimeline = query(
	'unchecked',
	async ({ cluster, resource }: { cluster: string; resource: string }): Promise<CapacityOwnerTimeline> => {
		const to = new Date();
		const from = new Date(to.getTime() - TRENDS_WINDOW_SECONDS * 1000);
		const bucket = TRENDS_BUCKET_SECONDS;
		const clusterClause = `ResourceAttributes['k8s.cluster.name'] = '${escapeClickHouseString(cluster)}'`;
		...
		try {
			...
			for (const r of allocRows) {
				allocByBucket.set(new Date(String(r.bucket)).toISOString(), Number(r.v) || 0);
			}

			// Group → owner key, accumulate per-bucket %, and track peak per owner.
			type Row = { t: string; key: string; kind: string; id: string; pct: number };
			const rows: Row[] = [];
			const peakByOwner = new Map<string, { kind: string; id: string; peak: number }>();
			for (const r of ownerRows) {
				...
				if (!prev || pct > prev.peak) peakByOwner.set(key, { kind, id, peak: pct });
			}

			// Top-N owners by peak; the rest collapse into "other".
			const ranked = [...peakByOwner.entries()].sort((a, b) => b[1].peak - a[1].peak);
			const topKeys = new Set(ranked.slice(0, OWNER_TIMELINE_TOP_N).map(([key]) => key));
			const owners = ranked
			...
			for (const row of rows) {
				const point = bucketMap.get(row.t) ?? { t: row.t, values: {}, other: 0 };
				if (topKeys.has(row.key)) point.values[row.key] = (point.values[row.key] ?? 0) + row.pct;
...
export const getCapacityOverview = query(async (): Promise<CapacityOverviewSummary> => {
	...
	if (observer.available) {
		...
		businessWork = await buildCapacityBusinessWork(observer.snapshot, context);
	}
	return {
		observer,
		businessWork
