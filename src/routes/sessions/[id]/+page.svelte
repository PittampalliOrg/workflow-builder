<script lang="ts">
	...
	async function forkFromEvent(sequence: number) {
		...
		try {
			...
			if (body.sessionId) goto(`/sessions/${body.sessionId}`);
		} finally {
			forking = false;
		}
	}
		...
		>
			<AlertDescription class="flex items-center flex-wrap gap-x-2 gap-y-1">
				...
				{#each expiringCreds as c, i (c.credId)}
					<a href="/vaults/{c.vaultId}" class="underline">{c.displayName}</a
			...
			{#if session}
				<Card>
					...
					<CardContent class="text-xs space-y-1">
						<div>
							<a href="/agents/{session.agentId}" class="text-primary hover:underline">
								{session.agentId}
							</a>
						</div>
				...
				{#if session.environmentId}
					<Card>
						...
						<CardContent class="text-xs space-y-1">
							<a
								href="/environments/{session.environmentId}"
					...
					<Card>
						...
						<CardContent class="text-xs space-y-1">
							{#each session.vaultIds as vid}
								<a href="/vaults/{vid}" class="text-primary hover:underline block truncate">
									{vid}
								</a>
							{/each}
						</CardContent>
