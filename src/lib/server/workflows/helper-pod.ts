...
export type HelperCmdResult = { exitCode: number; stdout: string; stderr: string };
...
export async function provisionWorkspaceHelperPod(
	executionId: string,
	suffix: string,
	opts?: { withGithubToken?: boolean; timeoutMinutes?: number; sharedWorkspaceKey?: string },
): Promise<{ baseUrl: string; token: string; githubToken: string | null } | null> {
	...
	const githubToken = opts?.withGithubToken ? await resolveWorkflowGithubToken() : null;

	// Fast path: already up.
	try {
		const ready = await waitForAgentWorkflowHostAppReady({ agentAppId: helperAppId });
	...
	try {
		const prov = await maybeProvisionAgentWorkflowHost({
			...
			sharedWorkspaceKey: opts?.sharedWorkspaceKey ?? `${executionId}__${suffix}`,
		});
...
export async function runHelperCommand(
	...
	const payload = JSON.stringify({ command, cwd, timeout: Math.ceil(budgetMs / 1000) });
