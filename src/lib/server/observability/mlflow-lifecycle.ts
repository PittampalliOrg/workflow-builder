...
async function getOrCreateLifecycleExperimentId(): Promise<string> {
	return getOrCreateMlflowExperimentId(lifecycleExperimentName(), "agent_lifecycle");
}
...
async function getOrCreateMlflowExperimentId(
	...
	if (!created.experiment_id) throw new Error("MLflow lifecycle experiment create returned no id");
...
async function createAgentVersionLoggedModel(params: {
	...
	const created = await mlflowRequest<LoggedModelResponse>("/api/2.0/mlflow/logged-models", {
		method: "POST",
		body: JSON.stringify({
			experiment_id: params.experimentId,
			name: modelName,
			...
			tags: [
				...
				{ key: "workflow_builder.agent_version_id", value: params.version.id },
				{ key: "workflow_builder.agent_version", value: String(params.version.version) },
				{ key: "workflow_builder.project_id", value: params.agent.projectId ?? "" },
			],
		}),
	});
...
export async function registerAgentVersionInMlflow(params: {
	agent: Agent;
	version: AgentVersion;
}): Promise<{ modelId: string; modelName: string | null; modelUri: string } | null> {
	if (!mlflowLifecycleEnabled()) return null;
	if (params.version.mlflowUri?.trim()) {
		return {
			modelId: params.version.mlflowUri.trim().replace(/^models:\//, "").replace(/^\//, ""),
...
export async function createWorkflowAgentMlflowRun(params: {
	...
	const experimentId = params.experimentId?.trim() || (await getOrCreateWorkflowExperimentId());
