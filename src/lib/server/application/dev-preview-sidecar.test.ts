...
describe("ApplicationDevPreviewSidecarService", () => {
	it("resolves the project-scoped env, then parses lastRun into the status view", async () => {
		const scStatus = {
			...
			data: {
				...
				lastSyncBytes: 2048,
				commands: ["deps", "test"],
				lastRun: { name: "test", exitCode: 0, durationMs: 900, executedIn: "app" },
			},
		};
		const sc = sidecar({ status: vi.fn(async () => scStatus) });
		const listEnvironments = vi.fn(async () => [env()]);
		const svc = new ApplicationDevPreviewSidecarService({ sidecar: sc, listEnvironments });
		...
		expect(sc.status).toHaveBeenCalledWith({ syncUrl: "http://10.0.0.1:9001/__sync" });
	...
	it("returns null when no env matches (route → 404)", async () => {
		...
		expect(
			await svc.status({ executionId: "exec-1", service: "workflow-builder", projectId: "p1" }),
		).toBeNull();
	});
