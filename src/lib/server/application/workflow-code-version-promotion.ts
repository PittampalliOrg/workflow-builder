...
export class ApplicationWorkflowCodeVersionPromotionService {
  ...
  async promote(
    ...
    const base =
      ...
      "main";
    const mode: SourceBundlePromotionMode =
      body.mode === "branch" ? "branch" : "pr";
    ...
    const baseRevision =
      readNonEmptyString(payload.sourceRevision) ?? undefined;
...
function normalizeSyncPaths(raw: unknown): string[] {
  ...
  const paths = raw.filter((path): path is string =>
    Boolean(readNonEmptyString(path)),
  );
