...
import type { CreateSessionRecordInput } from "$lib/server/application/ports";
...
export class PostgresOpenShellSessionStore {
	private readonly sessionRepository: CurrentSessionRepository;

	constructor(private readonly database: Database = defaultDb) {
		this.sessionRepository = new CurrentSessionRepository(database);
	}

	private async assertWorkflowExecutionInScope(
	...
	async resolveOpenShellTerminalTarget(
		sessionId: string,
		terminalId: string,
		caller: OpenShellCaller,
	...
	private async findAttachedSession(params: {
		caller: OpenShellCaller;
		workflowExecutionId?: string | null;
		sandboxName?: string | null;
	...
	async createOrAttachOpenShellSession(
		...
		if (requestedSessionId) {
			const existing = await this.getOpenShellSession(requestedSessionId, caller);
			if (!existing) {
		...
		const workflowExecutionId =
			typeof body.workflowExecutionId === "string"
				...
				: null;
		await this.assertWorkflowExecutionInScope(workflowExecutionId, caller);
		...
		const createInput: CreateSessionRecordInput = {
			agentId,
			agentVersion:
		...
		const created = await this.sessionRepository.createSession(createInput);
		if (sandboxName) {
			await this.sessionRepository.attachWorkspaceSandbox({
				sessionId: created.id,
				workspaceSandboxName: sandboxName,
			});
		}
...
const defaultStore = new PostgresOpenShellSessionStore();

export function getOpenShellSession(
	sessionId: string,
	caller: OpenShellCaller,
): Promise<OpenShellSessionView | null> {
	return defaultStore.getOpenShellSession(sessionId, caller);
}

export function resolveOpenShellTerminalTarget(
	sessionId: string,
	terminalId: string,
	caller: OpenShellCaller,
): Promise<{ sandboxName: string; terminalId: string; session: OpenShellSessionView } | null> {
	return defaultStore.resolveOpenShellTerminalTarget(sessionId, terminalId, caller);
}

export function createOrAttachOpenShellSession(
	body: Record<string, unknown>,
	caller: OpenShellCaller,
): Promise<OpenShellSessionView> {
	return defaultStore.createOrAttachOpenShellSession(body, caller);
}
