...
import { runScorersForRun } from "$lib/server/application/adapters/benchmark-score-runner";
import {
	agentVersions,
	agents,
	benchmarkArtifacts,
...
import { runDurableCascade } from "$lib/server/lifecycle/cascade";
import {
...
} from "$lib/server/benchmarks/agents";
...
} from "$lib/server/benchmarks/dapr-workflow-capacity";
import {
...
import { summarizeBenchmarkClusterPressure } from "$lib/server/benchmarks/cluster-pressure";
import { estimateBenchmarkEvaluationCapacity } from "$lib/server/benchmarks/evaluation-capacity";
import {
...
// $lib/server/benchmarks/service compat shim — keep working unchanged.
export {
	benchmarkAgentRuntimeCleanupInstanceIds,
	benchmarkInferenceStallRetryCount,
	benchmarkInferenceStallState,
...
} from "$lib/server/benchmarks/execution-plane";
...
const BENCHMARK_GRACEFUL_CANCELLATION_ENABLED = true;
const BENCHMARK_GRACEFUL_CANCELLATION_WAIT_SECONDS = 45;
...
type BenchmarkAgentRuntimeTarget = {
	runtimeAppId: string;
	instanceId: string;
};

type BenchmarkDurableCascadeCleanupResult = {
	allClosed: boolean;
	parentClosed: boolean;
	agentRuntimeClosed: boolean;
};

type DurableGracefulCancellationResult = "requested" | "alreadyGone" | "failed";

type BenchmarkDurableCascadeCleanupDeps = {
...
function shouldUseBenchmarkGracefulCancellation(): boolean {
	return parseBooleanFlag(
		env.BENCHMARK_GRACEFUL_CANCELLATION_ENABLED ??
			process.env.BENCHMARK_GRACEFUL_CANCELLATION_ENABLED,
		BENCHMARK_GRACEFUL_CANCELLATION_ENABLED,
	);
}

function benchmarkGracefulCancellationWaitMs(): number {
	return (
		clampInteger(
			env.BENCHMARK_GRACEFUL_CANCELLATION_WAIT_SECONDS ??
				process.env.BENCHMARK_GRACEFUL_CANCELLATION_WAIT_SECONDS,
			0,
			2 * 60,
			BENCHMARK_GRACEFUL_CANCELLATION_WAIT_SECONDS,
		) * 1000
	);
}
...
async function waitForDurableRuntimeClosed(
	...
	return waitForDurableRuntimeClosedWithin(
		label,
		fetchStatus,
		benchmarkTerminationWaitMs(),
		sleep,
	);
}

async function waitForDurableRuntimeClosedWithin(
	label: string,
	fetchStatus: () => Promise<unknown>,
	waitMs: number,
	...
	while (Date.now() < deadline) {
		...
		await sleepFn(Math.min(BENCHMARK_TERMINATION_WAIT_POLL_MS, remaining));
	}
	console.warn(
...
async function cleanupTerminalBenchmarkRun(params: {
	...
	if (params.outcome === "cancelled" && resourcesCleaned) {
...
export async function retryBenchmarkRunTerminalCleanupByRunId(runId: string) {
	const database = requireDb();
	const [run] = await database
		.select()
		.from(benchmarkRuns)
...
type DurableTerminationResult = "terminated" | "alreadyGone" | "closed" | "failed";

const benchmarkDurableCascadeCleanupDeps: BenchmarkDurableCascadeCleanupDeps = {
	getParentStatus: getBenchmarkWorkflowInstanceStatus,
	cancelParent: cancelBenchmarkWorkflowInstance,
	...
	cancelAgentRuntime: cancelBenchmarkAgentRuntimeInstance,
...
async function cleanupBenchmarkDurableWorkflowCascade(params: {
	...
	// Single source of truth lives in src/lib/server/lifecycle/cascade.ts. This
	// thin wrapper preserves the benchmark-specific deps/config so behavior is
	// identical to the pre-extraction implementation.
	return runDurableCascade({
		parentInstanceIds: params.parentInstanceIds,
		agentRuntimeTargets: params.agentRuntimeTargets,
		statePurgeInstanceIds: params.statePurgeInstanceIds,
		reason: params.reason,
		purge: params.purge,
		purgeGraceMs: params.purgeGraceMs,
		forceStatePurgeOnUnclosed: params.forceStatePurgeOnUnclosed,
		concurrency: params.concurrency ?? BENCHMARK_TERMINATION_CONCURRENCY,
		gracefulCancellationEnabled:
			params.gracefulCancellationEnabled ?? shouldUseBenchmarkGracefulCancellation(),
		gracefulCancellationWaitMs:
			params.gracefulCancellationWaitMs ?? benchmarkGracefulCancellationWaitMs(),
		deps: params.deps ?? benchmarkDurableCascadeCleanupDeps,
	});
}
...
async function cancelBenchmarkWorkflowInstance(
	instanceId: string,
	reason: string,
): Promise<DurableGracefulCancellationResult> {
	try {
		...
		if (!res.ok) {
			...
			console.warn(
				`Failed to request graceful benchmark workflow cancellation ${instanceId}: ${res.status} ${detail}`,
			);
			return "failed";
		}
		console.info(
			`Requested graceful benchmark workflow cancellation ${instanceId}`,
		);
		return "requested";
	} catch (err) {
		if (isBenignDaprTerminationMiss(err)) return "alreadyGone";
		console.warn(
			`Failed to request graceful benchmark workflow cancellation ${instanceId}:`,
			err instanceof Error ? err.message : err,
		);
		return "failed";
	}
...
async function cancelBenchmarkAgentRuntimeInstance(
	runtimeAppId: string,
	instanceId: string,
	reason: string,
): Promise<DurableGracefulCancellationResult> {
	try {
		...
		if (!res.ok) {
			...
			console.warn(
				`Failed to request graceful benchmark agent runtime cancellation ${runtimeAppId}/${instanceId}: ${res.status} ${detail}`,
			);
			return "failed";
		}
		console.info(
			`Requested graceful benchmark agent runtime cancellation ${runtimeAppId}/${instanceId}`,
		);
		return "requested";
	} catch (err) {
		if (isBenignDaprTerminationMiss(err)) return "alreadyGone";
		console.warn(
			`Failed to request graceful benchmark agent runtime cancellation ${runtimeAppId}/${instanceId}:`,
			err instanceof Error ? err.message : err,
		);
		return "failed";
	}
...
export const __benchmarkDurableRuntimeForTest = {
	...
	shouldUseBenchmarkGracefulCancellation,
...
async function cleanupStalledBenchmarkInstanceWorkflows(
	...
	try {
		...
		const durableCleanup = await cleanupBenchmarkDurableWorkflowCascade({
			parentInstanceIds: parentDaprInstanceId ? [parentDaprInstanceId] : [],
			agentRuntimeTargets: runtimeTargets,
			reason,
			purge: shouldPurgeBenchmarkDaprWorkflowsOnCleanup(),
			purgeGraceMs: 0,
		});
...
export function buildSwebenchInstanceWorkflowSpec(params: {
	...
	const checkoutCommand = [
		...
		"tmp_repo=/sandbox/repo.checkout.$$",
		"wait_seconds=0",
		"while ! mkdir \"$lock_dir\" 2>/dev/null; do",
		"  wait_seconds=$((wait_seconds + 1))",
		"  if [ \"$wait_seconds\" -ge 300 ]; then",
		"    echo \"Timed out waiting for SWE-bench checkout lock\" >&2",
		"    exit 1",
		"  fi",
		"  sleep 1",
		"done",
		"cleanup_checkout_lock() {",
		"  rm -rf \"$tmp_repo\"",
		"  rmdir \"$lock_dir\" 2>/dev/null || true",
		"}",
		"trap cleanup_checkout_lock EXIT",
		`if [ -d repo/.git ] && [ "$(git -C repo rev-parse HEAD 2>/dev/null || true)" = ${quoteShell(params.baseCommit)} ]; then`,
		"  cd repo",
		"  git status --short",
		"  exit 0",
		"fi",
		...
		"  cd /sandbox",
		"  rm -rf \"$tmp_repo\"",
		`  git clone --filter=blob:none --no-checkout ${quoteShell(`https://github.com/${params.repo}.git`)} "$tmp_repo"`,
		"  cd \"$tmp_repo\"",
