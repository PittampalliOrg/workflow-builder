 ...
 * read APIs.
 *
 * AUTH: normal in-environment callers may use INTERNAL_API_TOKEN. Cross-boundary
 * reads use a tuple-bound preview capability minted by the physical broker; the
 * adapter receives that leaf through `options.token` and sends it only in the
 * explicitly selected capability header. Preview environments never receive
 * the host's internal token.
 *
...
function configuredTailnetSuffix(): string | null {
  const value = (
    env.DEV_PREVIEW_TAILNET_SUFFIX ??
    process.env.DEV_PREVIEW_TAILNET_SUFFIX ??
    process.env.VCLUSTER_PREVIEW_TAILNET_SUFFIX ??
...
function exactTailnetFallback(target: PreviewRunTarget): string | null {
  if (!target.url) return null;
  const name = sanitizeBackingName(target.name);
  const suffix = configuredTailnetSuffix();
  if (!name || name !== target.name || !suffix) return null;
  try {
    const url = new URL(target.url);
    if (
      url.protocol !== "https:" ||
      url.hostname !== `wfb-${name}.${suffix}` ||
      url.port ||
      url.username ||
      url.password ||
      url.pathname !== "/" ||
...
export type HttpPreviewReadProxyOptions = {
  ...
  token?: string;
  /** Central broker uses the tuple leaf; normal service calls use internal auth. */
  authHeader?: "x-internal-token" | "x-preview-control-capability";
...
export class HttpPreviewReadProxy implements PreviewReadProxyPort {
  ...
  private timeoutMs(): number {
    const raw =
      this.options.timeoutMs ?? Number(env.PREVIEW_READ_PROXY_TIMEOUT_MS ?? "");
  ...
  private async request(
    ...
    if (!token)
      return failure("unauthorized", "INTERNAL_API_TOKEN not configured");
    const doFetch = this.options.fetchImpl ?? fetch;
    let response: Response;
    try {
      response = await doFetch(`${base}${path}`, {
        headers: {
          [this.options.authHeader ?? "x-internal-token"]: token,
        },
  ...
  > {
    ...
    if (!result.ok) return result;
    const rows = Array.isArray(result.data.executions)
  ...
  async listExecutionArtifacts(input: {
    ...
    if (!result.ok) return result;
    const rows = Array.isArray(result.data.artifacts)
  ...
  async fetchFileContent(input: {
    ...
    PreviewReadResult<{ bytes: Buffer; contentType: string | null }>
  > {
    ...
    if (Number.isFinite(declared) && declared > maxBytes) {
      return failure(
        "bad-response",
        `content too large (${declared}B > ${maxBytes}B)`,
      );
    }
    let bytes: Buffer;
    try {
