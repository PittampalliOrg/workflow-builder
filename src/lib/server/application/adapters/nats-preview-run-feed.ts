...
function statusFromEventType(eventType: string): PreviewRunEvent["status"] {
...
export function decodePreviewRunEvent(
	raw: unknown,
	preview: PreviewRunTarget,
): PreviewRunEvent | null {
...
export type NatsPreviewRunFeedDeps = {
 ...
 */
export class NatsPreviewRunFeed implements PreviewRunFeedPort {
	constructor(private readonly deps: NatsPreviewRunFeedDeps = {}) {}
				...
				try {
					...
					(async () => {
						for await (const msg of messages) {
							try {
								const decoded = decodePreviewRunEvent(msg.json(), preview);
