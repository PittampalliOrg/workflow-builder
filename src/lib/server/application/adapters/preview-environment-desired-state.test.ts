...
const API_PATH = '/apis/preview.stacks.io/v1alpha1/namespaces/preview-system/previewenvironments';
...
function resource(overrides: Record<string, unknown> = {}): Record<string, any> {
...
describe('KubernetesPreviewEnvironmentDesiredStateAdapter', () => {
	it('uses only the explicit least-privilege hub kubeconfig transport', async () => {
		expect(() => previewEnvironmentHubKubeFetch({})).toThrow('PREVIEW_ENVIRONMENT_HUB_KUBECONFIG');
		const remote = vi.fn(async () => json({ reason: 'NotFound' }, 404));
		const fetch = previewEnvironmentHubKubeFetch(
			{
				PREVIEW_ENVIRONMENT_HUB_KUBECONFIG: '/var/run/preview-environment-hub/kubeconfig',
	...
	it('authors the exact immutable reconciler contract before returning', async () => {
		...
		expect(created).toEqual(buildPreviewEnvironmentDesiredStateManifest(command));
	...
	it('makes same-contract 409 and ambiguous create retries idempotent', async () => {
		const retry = new KubernetesPreviewEnvironmentDesiredStateAdapter({
			fetch: vi.fn(async (_path, init = {}) =>
				init.method === 'POST' ? json({ reason: 'AlreadyExists' }, 409) : json(resource())
			)
		});
		await expect(retry.create(command)).resolves.toMatchObject({ uid: 'uid-1' });
	...
	it('rejects a different-contract name collision', async () => {
		...
		const conflict = new KubernetesPreviewEnvironmentDesiredStateAdapter({
			fetch: vi.fn(async (_path, init = {}) =>
				init.method === 'POST' ? json({ reason: 'AlreadyExists' }, 409) : json(colliding)
			)
		});
...
describe('DesiredStateVclusterPreviewGateway', () => {
	...
	it('waits for desired-state finalizers before SEA down', async () => {
		const order: string[] = [];
		const desired = {
			create: vi.fn(),
			inspect: vi.fn(),
		...
		expect(order).toEqual(['desired:absent', 'sea:down']);
	});
	...
	it('compensates a failed SEA up in the same safe order', async () => {
		...
		await expect(adapter.provision(launchInput())).rejects.toThrow('was compensated');
		expect(order).toEqual(['desired:absent', 'sea:down']);
	});
	...
	it('preserves both the provision and compensation failures', async () => {
		...
		const error = await adapter.provision(launchInput()).catch((cause) => cause);
...
describe('BrokeredVclusterPreviewGateway', () => {
	it('constructs and serves reads without hub desired-state credentials', async () => {
		...
		await expect(adapter.listWithCounts()).resolves.toMatchObject({ previews: [] });
	...
	it('sends guarded teardown only to the authenticated physical broker', async () => {
		...
		await expect(adapter.teardown(command.name, guard)).resolves.toEqual(sparseSeaReceipt);
	...
	it('refuses unguarded destructive commands before network access', async () => {
		...
		await expect(adapter.teardown(command.name, undefined as never)).rejects.toBeInstanceOf(
			PreviewEnvironmentDesiredStateOwnershipError
		);
		expect(brokerFetch).not.toHaveBeenCalled();
	});
	...
	it.each([
	...
	])('rejects a compromised physical teardown receipt: %s', async (_case, change) => {
		const guard = {
			mode: 'owned' as const,
			requestId: command.provenance.requestId,
			sourceRevision: command.sourceRevision
		...
		await expect(adapter.teardown(command.name, guard)).rejects.toBeInstanceOf(
			PreviewEnvironmentDesiredStateOwnershipError
		);
	});
