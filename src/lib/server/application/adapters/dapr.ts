...
export class DaprWorkflowScheduler implements WorkflowScheduler {
  async startSwWorkflow(
    ...
    const response = await daprFetch(
      `${input.orchestratorUrl}/api/v2/sw-workflows`,
      {
        method: "POST",
  ...
  async startScriptWorkflow(
    ...
    const scriptSha256 = createHash("sha256")
      .update(input.script, "utf8")
      .digest("hex");
    const response = await daprFetch(
      `${input.orchestratorUrl}/api/v2/script-workflows`,
      {
        method: "POST",
        headers: input.headers,
        body: JSON.stringify({
...
export class DaprEventBus implements EventBus {
  constructor(
    private readonly pubsubComponent = env.DAPR_PUBSUB_COMPONENT ||
      "workflow-triggers-pubsub",
  ) {}
...
export class DaprCredentialStore implements CredentialStore {
  ...
  async resolveSecret(
    ...
    if (!response.ok) {
      const detail = await response.text().catch(() => "");
      throw new Error(
        `Dapr secret lookup failed (${response.status}): ${detail}`,
      );
    }
