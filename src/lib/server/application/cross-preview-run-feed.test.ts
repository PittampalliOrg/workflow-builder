...
describe("ApplicationCrossPreviewRunFeedService", () => {
	it("streams the preview list and per-preview run events as SSE", async () => {
		const runEvent: CrossPreviewRunEvent = {
			previewName: "pr-1",
			previewUrl: null,
		...
		const feed: CrossPreviewRunFeedPort = {
			async subscribe({ onEvent }) {
				onEvent(runEvent);
				return async () => {};
			},
		};
		const service = new ApplicationCrossPreviewRunFeedService({
			feed,
