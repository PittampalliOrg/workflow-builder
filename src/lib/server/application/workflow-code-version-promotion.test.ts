...
describe("ApplicationWorkflowCodeVersionPromotionService", () => {
	...
	beforeEach(() => {
		workflowData = {
			...
			getWorkflowArtifactForExecution: vi.fn(async () => sourceBundleArtifact()),
	...
	it("promotes a source bundle and records durable promotion metadata", async () => {
		await expect(
			service.promote({
				...
				userId: "user-1",
				projectId: "project-1",
				body: { mode: "branch", repo: "https://github.com/body/repo.git" },
			}),
	...
	it("rejects non-source-bundle artifacts", async () => {
		vi.mocked(workflowData.getWorkflowArtifactForExecution).mockResolvedValueOnce(
			sourceBundleArtifact({ kind: "markdown" }),
		);
	...
	it("returns the existing bad request when no target repo can be resolved", async () => {
		...
		await expect(
			...
			status: "error",
			httpStatus: 400,
			message: "Target repo could not be resolved — pass { repo: 'owner/name' }",
		});
		expect(runner.promoteSourceBundle).not.toHaveBeenCalled();
	});
	...
	it("maps command marker errors to 502 JSON without metadata persistence", async () => {
		...
		await expect(
			...
			status: "ok",
			httpStatus: 502,
			body: { ok: false, error: "no_github_token", output: "ERR=no_github_token\n" },
		});
