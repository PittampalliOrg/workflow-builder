...
export type CrossPreviewRunFeedDeps = {
	feed: CrossPreviewRunFeedPort;
	/** Lists the previews to tail (ready Tier-2 vclusters), name + browsable url. */
	listPreviews: () => Promise<CrossPreviewTarget[]>;
 ...
 */
export class ApplicationCrossPreviewRunFeedService {
	constructor(private readonly deps: CrossPreviewRunFeedDeps) {}
		...
		return new ReadableStream<Uint8Array>({
			async start(controller) {
				...
				async function resubscribe() {
					if (cancelled) return;
					let previews: CrossPreviewTarget[];
					try {
					...
					unsubscribe = await deps.feed.subscribe({
						previews,
						onEvent: (event: CrossPreviewRunEvent) => send("run", event),
