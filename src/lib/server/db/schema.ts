...
export const workflowExecutions = pgTable(
  "workflow_executions",
  {
    ...
    summaryOutput: jsonb("summary_output").$type<Record<
      string,
      unknown
    > | null>(),
...
/** Immutable cross-database handoff from an isolated preview to physical control. */
export const previewControlArtifacts = pgTable(
...
export type PreviewControlArtifactRow = InferSelectModel<
...
export type PreviewAcceptedImageReceiptRow = InferSelectModel<
...
export const workflowWorkspaceSessions = pgTable(
  "workflow_workspace_sessions",
  {
    ...
    clonePath: text("clone_path"),
    backend: text("backend").notNull().$type<"openshell" | "juicefs">(),
...
// Workspace API keys. project_id remains nullable so pre-workspace webhook
// credentials retain their owner-scoped behavior during migration.
export const apiKeys = pgTable(
  ...
  keyHash: text("key_hash").notNull(), // Store hashed version of the key
    keyPrefix: text("key_prefix").notNull(), // Store first few chars for display (e.g., "wfb_abc...")
  createdAt: timestamp("created_at").notNull().defaultNow(),
...
export type AgentSkillRegistrySourceType = "registry" | "profile" | "custom";
...
export type NewResourcePromptVersion =
  typeof resourcePromptVersions.$inferInsert;
...
export const prPreviews = pgTable("pr_previews", {
  ...
  authority: jsonb("authority").$type<Record<string, unknown> | null>(),
  error: text("error"),
...
export type BenchmarkResourceLeaseType =
  ...
  | "evaluator_slot"
  | "model_slot";
export type BenchmarkResourceLeaseStatus = "active" | "released" | "expired";
...
export const gitopsActivityEvents = pgTable(
  "gitops_activity_events",
  {
    ...
    activityType: text("activity_type")
      .notNull()
      .$type<GitOpsActivityType | string>(),
    phase: text("phase"),
    reason: text("reason"),
    message: text("message"),
...
export const codeFunctions = pgTable("code_functions", {
  ...
  compositionGraph:
    jsonb("composition_graph").$type<CodeFunctionCompositionGraph>(),
 ...
 */
export const codeFunctionRevisions = pgTable(
...
export const codeFunctionRevisionsRelations = relations(
...
export type NewBenchmarkRunInstance = typeof benchmarkRunInstances.$inferInsert;
export type EnvironmentImageBuild = typeof environmentImageBuilds.$inferSelect;
...
export type NewEvaluationDatasetRow = typeof evaluationDatasetRows.$inferInsert;
