<script lang="ts">
	...
	import { Target, Pause, Play, CheckCheck, Loader2, Plus, Terminal, X } from '@lucide/svelte';

	interface Props {
		sessionId: string;
		/**
		 * interactive-cli runtimes (claude-code-cli/codex-cli/agy-cli) drive their
		 * OWN native `/goal` loop inside the vendor CLI. For them this card becomes
		 * a thin launcher that types `/goal <objective>` into the terminal — there
		 * is no BFF-tracked goal row, budget, or iteration cap to show.
		 */
		isCli?: boolean;
	}

	let { sessionId, isCli = false }: Props = $props();
	...
	// CLI native-goal launcher: short-lived "typed /goal into the terminal" note.
	let cliSentObjective = $state<string | null>(null);
	...
	$effect(() => {
		void sessionId;
		// CLI runtimes have no BFF goal row to poll — the native /goal state lives
		// in the terminal. Skip the polling loop entirely.
		if (isCli) return;
		load();
	...
	async function submitGoal() {
		...
		try {
			...
			// CLI: don't send budget/iterations — native /goal takes only the
			// objective (bound, if needed, via the condition text itself).
			const payload = isCli ? { objective: objective.trim() } : body;
			const res = await fetch(`/api/v1/sessions/${sessionId}/goal`, {
				method: 'POST',
			...
			if (isCli) {
				// Native path returns { native: true } — the loop runs inside the CLI.
				cliSentObjective = objective.trim();
				dialogOpen = false;
				return;
			}
	...
	async function patchStatus(status: 'paused' | 'complete') {
		...
		try {
			...
			if (isCli) {
				// Native path returns { native: true } — typed `/goal clear`.
				if (res.ok) cliSentObjective = null;
				return;
			}
...
<div class="space-y-2 rounded-lg border p-3" data-testid="session-goal-badge">
	<div class="flex items-center justify-between gap-2">
		...
		{#if isCli}
			<Badge
				variant="outline"
				class="text-[10px] border-violet-500/30 bg-violet-500/15 text-violet-700 dark:text-violet-300"
			>
				Native /goal
			</Badge>
		{:else if goal && statusInfo}
	...
	{#if isCli}
		<p class="text-[11px] text-muted-foreground">
			This CLI runs its own native <code class="text-[10px]">/goal</code> loop. Setting an objective
			types <code class="text-[10px]">/goal …</code> into the terminal; watch progress in the
			Terminal tab. The CLI's own evaluator decides when it's done.
		</p>
		{#if cliSentObjective}
			<p class="line-clamp-2 rounded bg-muted/50 px-2 py-1 text-[11px]" title={cliSentObjective}>
				<span class="text-muted-foreground">Sent:</span>
				<code class="text-[10px]">/goal {cliSentObjective}</code>
			</p>
		{/if}
		<div class="flex flex-wrap gap-1.5 pt-1">
			<Button size="sm" variant="outline" class="h-7 text-xs" disabled={busy} onclick={() => openDialog(null)}>
				<Terminal class="size-3" /> {cliSentObjective ? 'Replace /goal' : 'Set /goal'}
			</Button>
			{#if cliSentObjective}
				<Button size="sm" variant="outline" class="h-7 text-xs" disabled={busy} onclick={() => patchStatus('complete')}>
					<X class="size-3" /> Clear /goal
				</Button>
			{/if}
		</div>
	{:else if goal && statusInfo}
...
<Dialog.Root bind:open={dialogOpen}>
	<Dialog.Content class="sm:max-w-lg">
		<Dialog.Header>
			<Dialog.Title>
				{#if isCli}
					{cliSentObjective ? 'Replace /goal' : 'Set a native /goal'}
				{:else}
			...
			<Dialog.Description>
				{#if isCli}
					Types <code class="text-[10px]">/goal &lt;objective&gt;</code> into this CLI's terminal and
					hands the loop to the CLI's own native goal harness — it keeps working across turns and an
					evaluator decides completion. Bound it in the text if you want (e.g. “… or stop after 20
					turns”). Stop anytime with <code class="text-[10px]">/goal clear</code>.
				{:else}
		...
		<Dialog.Footer>
			...
			<Button onclick={submitGoal} disabled={busy}>
				...
				{isCli ? 'Set /goal' : 'Start goal loop'}
			</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
