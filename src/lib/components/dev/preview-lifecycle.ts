...
export function expiresIn(
	...
	if (mins < 60) return { label: `expires in ${mins}m`, urgent: true, expired: false };
	const hours = Math.floor(mins / 60);
	if (hours < 24) return { label: `expires in ${hours}h`, urgent: false, expired: false };
	return { label: `expires in ${Math.floor(hours / 24)}d`, urgent: false, expired: false };
}

type PreviewLike = Pick<VclusterPreviewSummary, "phase" | "state" | "protected" | "pool" | "origin">;

/** The status string that drives the row badge: a slept preview reads "slept"
 * even if its underlying Job phase still says "ready". */
export function effectivePreviewStatus(preview: Pick<PreviewLike, "phase" | "state">): string {
	return preview.state === "slept" ? "slept" : preview.phase;
}

/** The shared tone for a preview's effective status (feeds the pill/meter). */
export function phaseTone(preview: Pick<PreviewLike, "phase" | "state">): StatusTone {
 ...
 * SEA sleep 409 contract: protected previews and pool-backed members refuse, and
 * PR previews are torn down by their PR closing rather than slept by hand.
 */
export function sleepDisabledReason(
	preview: Partial<Pick<PreviewLike, "state" | "protected" | "pool" | "origin">>,
): string | null {
	if (preview.state === "slept") return "Already sleeping";
	if (preview.protected) return "Protected preview — exempt from sleep/eviction";
	if (preview.pool) return "Backed by a warm-pool member (stays claim-ready)";
	if (preview.origin === "pr") return "PR previews sleep automatically when the PR closes";
	return null;
}
