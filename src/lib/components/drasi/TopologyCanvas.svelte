<script lang="ts">
	...
	// XYFlow 1.5.2's NodeWrapper renders `node.ariaRole` and spreads
	// `node.domAttributes` onto the `.svelte-flow__node` element, but it does
	// NOT render the extra `node.ariaLabel` field — so the accessible name
	// goes through `domAttributes`. The public `domAttributes` type omits
	// `aria-label`, hence the narrow double assertion.
