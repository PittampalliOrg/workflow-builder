...
<div class="min-h-0 flex-1 overflow-auto">
	<div class="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-3 @max-md:gap-2 @max-md:p-3">
		{#each sources as source (source.nodeId)}
			...
			>
				<!-- The id/title block owns the full card width; Inspect stacks on its
					own row so long source ids (workflow-builder-postgres, …-v2) wrap
					readably instead of collapsing into one-character columns. -->
				...
				<!-- Extreme narrow containers (~166px): label/value pairs stack so
					ids, badges, and status text keep their full measure instead of
					squeezing into clipped one-line columns. -->
				<dl class="min-w-0 space-y-1.5 text-xs">
					<div
						class="flex min-w-0 items-center justify-between gap-3 @max-md:flex-col @max-md:items-start @max-md:gap-0.5"
					>
						<dt class="text-muted-foreground">Readiness</dt>
						<dd class="min-w-0 max-w-full"><ObservedChip status="unavailable" /></dd>
					</div>
					<div
						class="flex min-w-0 items-center justify-between gap-3 @max-md:flex-col @max-md:items-start @max-md:gap-0.5"
					>
						<dt class="text-muted-foreground">Phase distribution</dt>
						<dd class="min-w-0 max-w-full break-words text-muted-foreground">Unavailable</dd>
					</div>
				</dl>
					...
					<div class="min-w-0">
						...
						<div class="flex min-w-0 flex-wrap gap-1">
							{#each source.scope as kind (kind)}
								<Badge
									variant="outline"
									class="h-auto min-h-5 max-w-full break-all px-1.5 py-0.5 text-[0.65rem]"
									>{kind}</Badge
								>
							{/each}
						</div>
