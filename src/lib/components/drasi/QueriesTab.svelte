	...
	let filtered = $derived.by(() => {
		...
		return queries.filter((query) => {
			...
			return (
				...
				query.logicalId.toLowerCase().includes(needle) ||
...
<div class="flex min-h-0 flex-1 flex-col">
	<div
		class="flex flex-wrap items-center gap-2 border-b px-5 py-2.5 @max-md:flex-col @max-md:items-stretch @max-md:px-3"
	>
		<div class="relative min-w-[180px] flex-1 sm:max-w-xs @max-md:w-full @max-md:min-w-0 @max-md:max-w-none">
			<Search
		...
		<!-- Narrow containers: the segmented control cannot fit long source ids,
			so substitute a compact native select with the same semantics. The
			select stays until the container fits the full segmented group
			(~512px / @lg), not merely 448px. -->
		<label
			class="flex items-center gap-1.5 text-[0.7rem] text-muted-foreground @lg:hidden"
		>
		...
		<div
			class="inline-flex items-center gap-1 rounded-lg bg-muted p-[3px] text-muted-foreground @max-lg:hidden"
			role="group"
			aria-label="Filter by source"
		>
			<button
		...
		<span class="ml-auto text-[0.7rem] text-muted-foreground @max-md:ml-0">
	...
	<div class="min-h-0 flex-1 overflow-auto @md:hidden">
		{#if filtered.length === 0}
			<div class="flex h-full flex-col items-center justify-center gap-2 px-4 py-10 text-center">
				<p class="text-sm text-muted-foreground">
					No continuous queries match the current filters.
				</p>
				{#if hasFilters}
		...
		{:else}
			<ul class="divide-y">
				{#each filtered as query (query.id)}
					<li class="flex min-w-0 flex-col gap-2 px-4 py-3">
						<!-- The name/id block owns the full row width; the Inspect action
							stacks on its own row so extreme narrow containers never squeeze
							identifiers into one-character columns. -->
						<div class="min-w-0">
							<p class="break-words text-xs font-medium">{query.name}</p>
							<p class="break-all font-mono text-[0.65rem] text-muted-foreground">
								{query.id}
							</p>
						</div>
						<dl class="space-y-1 text-[0.7rem]">
							<div class="flex items-start justify-between gap-3">
								<dt class="shrink-0 text-muted-foreground">Source</dt>
								<dd
							...
							<div class="flex items-start justify-between gap-3">
								<dt class="shrink-0 text-muted-foreground">Condition</dt>
								<dd class="text-right">{query.condition}</dd>
							</div>
						...
						<Button
							variant="ghost"
							size="sm"
							class="h-7 gap-1 self-start px-2 text-xs"
							onclick={() => onInspect(query.id)}
							aria-label="Inspect {query.name}"
						>
							Inspect
	...
	<p class="border-t px-5 py-2 text-[0.7rem] leading-relaxed text-muted-foreground @max-md:px-3">
