					...
					{#if !collapsedTraceIds.has(group.traceId)}
						<div>
							{#each group.spans as span (rowKey(span))}
								...
								<button
									class={`border-t border-white/[0.04] px-4 py-3 text-left transition-colors hover:bg-white/[0.04] ${selectedSpan?.traceId === span.traceId && selectedSpan?.spanId === span.spanId ? 'bg-orange-500/10' : ''}`}
								...
								>
									<div class="grid gap-3 xl:grid-cols-[minmax(0,1.8fr)_120px_78px_96px_minmax(180px,1fr)] xl:items-center">
										...
										<div class="flex flex-wrap items-center gap-3 text-[11px] xl:contents">
											...
											<div class="relative my-1 h-6 rounded-full bg-white/[0.05] xl:my-0">
												<div
