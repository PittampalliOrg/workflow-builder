...
export function quantityRatios(
...
): { used: number; reserved: number; free: number; over: number; nominal: number; usedAbs: number; reservedAbs: number } {
	const usedAbs = parseQuantity(used);
	const reservedAbs = parseQuantity(reserved);
	...
	if (nominalAbs <= 0) {
		...
		const total = usedAbs + reservedAbs;
		return {
			used: total > 0 ? 100 : 0,
			reserved: 0,
			free: 0,
	...
	const overPct =
		usedAbs + reservedAbs > nominalAbs
			? Math.min(100, ((usedAbs + reservedAbs - nominalAbs) / nominalAbs) * 100)
			: 0;
...
export function formatQuantityForResource(resource: string, abs: number): string {
	if (abs === 0) return '0';
	if (resource === 'cpu') {
