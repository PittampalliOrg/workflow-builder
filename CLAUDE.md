...
- MLflow Traces UI shows `IN_PROGRESS` after a successful workflow → **known cosmetic-only limitation** in MLflow OSS 3.10.x. Our orchestrator's Python spans are parented to `durabletask-go` engine spans that MLflow's OTLP receiver drops (no `gen_ai.*` attrs), so MLflow never sees a `parent_id=None` span to flip state. MLflow's REST API does **not** expose a state-update endpoint (verified — `PATCH/PUT/POST` on `/api/3.0/mlflow/traces/{id}` all return 404/405; `StartTraceV3` errors with "blank-out primary key column" when called on an existing trace). Spans + tags + duration are all queryable; only the badge is wrong. ~92% of runs hit this (11 of 12 recent `wf_async_coding_task_v1` runs were IN_PROGRESS; remaining one was a flaky-finalize outlier). Real fixes need either (a) a CronJob `UPDATE trace_info SET state='OK' WHERE state='IN_PROGRESS' AND request_time < now() - interval '10 min'` in mlflow ns, or (b) Tailscale egress to mlflow-postgres from orchestrator + inline SQL. The REST-API attempt was tried and reverted (commit `c36747e7` → revert `854f0828`, 2026-05-12).
