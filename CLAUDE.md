...
- **Workflow Engine**: Dapr Workflow SDK (Python, `dapr-ext-workflow==1.18.0`; dapr-agent-py `>=1.18,<1.19`) via workflow-orchestrator; Dapr runtime + control plane **1.18.1** (daprd/operator/placement/sentry). NOTE: 1.18's workflow payload-size pre-check is active — an over-budget dispatch (>95% of `dapr.io/max-body-size`, 16Mi on sandbox+orchestrator pods) is SILENTLY not sent to the SDK (no `ResourceExhausted`, durable); confirm via the `dapr_runtime_workflow_(activity_)payload_size_ratio` metric (≥0.95 trips the stall)
...
- **Completion contract**: goals are authored in code via the dynamic-script engine (goalSpec/evidence) and completed by the BFF **evidence backstop** (`evaluateGoalCompletion` on each idle) or budget/iteration caps. The `create_goal`/`update_goal`/`get_goal` MCP tools and their auto-wire (`GOAL_MCP_AUTO_WIRE`) were **REMOVED** — agents no longer self-declare/self-complete goals over MCP. Non-MCP authoring stays: `POST/PATCH /api/v1/sessions/[id]/goal` (Goal card) + `POST /api/internal/goals/[sessionId]/evaluate` (completion authority). `workflow-mcp-server` still hosts the workflow/script/trace/team tools.
...
- **Reviewer/agent wedged in an MCP tool call (`tool_activity.started` with no `tool_result`, no error, for minutes)** → upstream MCP `ClientSession` sets no per-call timeout, so a stalled streamable-HTTP response (SSE stream kept alive while the final result never arrives — e.g. a slow trace tool over dev→hub egress) hangs the `run_tool` activity forever. `src/mcp_tool_timeout.py` installs a process-wide total per-request read timeout (`DAPR_AGENT_MCP_TOOL_TIMEOUT_SECONDS`, default 180, `0` disables) so the call errors out instead. Raise it if a runtime hosts legitimately long MCP tools
