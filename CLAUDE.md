...
- **Runtime registry (SSOT)** — `services/shared/runtime-registry.json` is the single source of truth for the 5 agent runtimes (`dapr-agent-py`, `claude-agent-py`, `adk-agent-py`, `browser-use-agent`, `claude-code-cli`): per-runtime identity (app-id/image/container) + capability descriptor (durability granularity incl. `per-session`, MCP/hooks/permission support, providers, `interactiveTerminal`, `executionClass`, `cliAuth`). `scripts/sync-runtime-registry.mjs` generates the orchestrator + BFF build-context copies (drift-guarded).
