...
Post 2026-04-19 function-router cutover (legacy `workspace-runtime` TS service decommissioned, `workspace/*` routed to `openshell-agent-runtime` which is stateless w.r.t. this table), the orchestrator owns the DB write for `workflow_workspace_sessions`:
